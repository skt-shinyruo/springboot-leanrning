...
来源：https://zhuanlan.zhihu.com/p/456835010
抓取时间：2026-03-07T07:39:42.146Z
...
1. `ChannelFuture f = serverBootstrap.bind(PORT).sync()`这一步会是下篇文章要重点分析的主题`Main Reactor Group`的启动，绑定端口地址，开始监听客户端连接事件（`OP_ACCEPT`）。本文我们只关注创建流程。
2. `f.channel().closeFuture().sync()`等待服务端`NioServerSocketChannel`关闭。Netty服务端到这里正式启动，并准备好接受客户端连接的准备。
3. `shutdownGracefully`优雅关闭`主从Reactor线程组`里的所有`Reactor线程`。
...
![图9](images/img_009.jpg)

## 多种NIO的实现

Common Linux Mac

![图10](images/img_010.jpg)
