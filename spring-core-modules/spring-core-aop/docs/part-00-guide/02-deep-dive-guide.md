...
上一章：[01. 主线时间线：Spring Core AOP](01-mainline-timeline.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[01. AOP 心智模型：代理（Proxy）+ 入口（Call Path）](../part-01-proxy-fundamentals/01-aop-proxy-mental-model.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
- 只看关注的 beanName（例如 `tracedBusinessService`、`selfInvocationExampleService`）
- 或只看关注的包名（例如 `com.learning.springboot.springcoreaop`）
...
> 想把 AOP 放回容器视角（AutoProxyCreator/Advisor 主线）：见 [07. autoproxy-creator-mainline](../part-02-autoproxy-and-pointcuts/01-autoproxy-creator-mainline.md)。
...
而 `@Around` 通常会落到类似下面的类（名称可能随版本略有变化）：
...
- pointcut 表达式系统：见 [08. pointcut-expression-system](../part-02-autoproxy-and-pointcuts/02-pointcut-expression-system.md)
...
- 能在 `interceptorsAndDynamicMethodMatchers` 里看到链条顺序
- 能解释 `@Order(1)` 与 `@Order(2)` 谁在外层、谁先执行
...
- 多代理叠加与顺序：见 [09. multi-proxy-stacking](../part-03-proxy-stacking/01-multi-proxy-stacking.md)
- 真实项目叠加 Debug Playbook：见 [10. real-world-stacking-playbook](../part-03-proxy-stacking/02-real-world-stacking-playbook.md)

跑：`SpringCoreAopLabTest#tracedBusinessServiceIsAnAopProxy`

- 能在断点里看到：目标对象 → 经过 AutoProxyCreator → 最终变成 proxy
- 能解释：为什么最终注入到业务里的 bean 可能不是所写的那个 class
...
- 能用断言证明：未授权会被阻断、缓存命中会短路、目标方法内事务处于激活状态
- 能在断点里复述：链条组装 → proceed 嵌套 → 哪一层负责鉴权/缓存/事务
