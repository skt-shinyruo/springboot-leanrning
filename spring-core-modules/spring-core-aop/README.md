...
0. [深挖指南：把“代理产生 + advice 链执行”落到源码与断点](docs/part-00-guide/02-deep-dive-guide.md)
...
7. [AOP 的容器主线：AutoProxyCreator 作为 BPP（Advisor/Advice/Pointcut）](docs/part-02-autoproxy-and-pointcuts/01-autoproxy-creator-mainline.md)
8. [Pointcut 表达式系统：execution/within/this/target/args/@annotation/...](docs/part-02-autoproxy-and-pointcuts/02-pointcut-expression-system.md)
9. [多切面/多代理叠加与顺序：AOP/Tx/Cache/Security](docs/part-03-proxy-stacking/01-multi-proxy-stacking.md)
...
| AutoProxyCreator 为什么是 BPP | [docs/07](docs/part-02-autoproxy-and-pointcuts/01-autoproxy-creator-mainline.md) | `SpringCoreAopAutoProxyCreatorInternalsLabTest` | 代理何时产生、Advisor 如何筛选、为什么这个 bean 会/不会被代理 |
| pointcut 最常见误判（this vs target） | [docs/08](docs/part-02-autoproxy-and-pointcuts/02-pointcut-expression-system.md) | `SpringCoreAopPointcutExpressionsLabTest` | 为什么同一表达式在 JDK/CGLIB 下命中不同、如何用断言验证 |
| 并发/性能边界（ThreadLocal 不串线） | [docs/11](docs/part-02-perf-concurrency/01-proxy-concurrency-perf.md) | `SpringCoreAopProxyConcurrencyLabTest` + `CorrelationIdAspect` | 为什么 proxy 可并发调用，但 per-invocation 状态必须线程隔离/可清理 |
| 多 advisor vs 多层 proxy（套娃） | [docs/09](docs/part-03-proxy-stacking/01-multi-proxy-stacking.md) | `SpringCoreAopMultiProxyStackingLabTest` | “叠加”到底是什么形态、顺序问题如何分流定位 |
| 真实叠加（Tx/Cache/Security）排障 | [docs/10](docs/part-03-proxy-stacking/02-real-world-stacking-playbook.md) | `SpringCoreAopRealWorldStackingLabTest` | 真实基础设施下如何用断点与断言定位“不生效/被绕过/短路/顺序怪” |
