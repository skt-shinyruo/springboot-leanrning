...
本章围绕「01. 事务边界（Transaction Boundary）：你到底在“保护”哪一段代码？」展开，目标是把机制边界写成可回归的事实（可运行入口与关键观察点会在文中给出）。
...
2. **回滚与否取决于异常是否“逃逸出边界”**
   - 抛出运行时异常并向外传播：默认回滚
   - 异常被你 catch 住了：默认会提交（除非你显式标记 rollback-only，见 [90. common-pitfalls](../appendix/01-common-pitfalls.md)）
