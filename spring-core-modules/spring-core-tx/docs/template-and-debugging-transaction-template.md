...
<!-- CHAPTER-CARD:START -->
!!! summary "章节学习卡片（五问闭环）"
    本章围绕程序化事务：为什么 `TransactionTemplate` 在学习阶段很有价值？展开，主线可以概括为：方法调用 → 事务拦截器 → 获取/创建事务（TransactionManager）→ 绑定资源到线程 → 正常提交/异常回滚；传播决定“加入还是新开”。
...
上一章：[04. 传播行为（Propagation）：`REQUIRED` vs `REQUIRES_NEW` 到底差在哪？](transaction-basics-propagation.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[02. Debug / 观察：如何判断“当前是否真的有事务”？](template-and-debugging-debugging.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
- 能清楚看到：什么时候提交、什么时候标记回滚
- 能把“事务边界”写成显式代码（帮助建立直觉）
...
上一章：[04-propagation](transaction-basics-propagation.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[06-debugging](template-and-debugging-debugging.md)
