# 01. CTW：Compile-Time Weaving（编译期织入）
<!-- CHAPTER-CARD:START -->
!!! summary "章节学习卡片（五问闭环）"
    本章围绕CTW：Compile-Time Weaving（编译期织入）展开，主线可以概括为：代理 vs 织入：选择 LTW/CTW → 定义切点（execution/call/...）→ weaving 生效取决于 classloader/agent/时机 → 用测试/断点验证。
...
上一章：[01. LTW：Load-Time Weaving（-javaagent）](../part-02-ltw/01-ltw-basics.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[01. Join Point & Pointcut Cookbook（速查）](../part-04-join-points/01-join-point-cookbook.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
命令行 `mvn test` 一切正常，但在 IDE 单跑时 advice 不触发

CTW 的关键是“构建产物是否被织入”；IDE 的编译/运行配置如果绕过了 maven weaving，可能跑的是未织入 class
...
以 maven 构建产物为准（先跑 `mvn -pl :spring-core-aop-weaving test`）；IDE 场景需要确保复用 maven 编译输出或启用相同 weaving 配置

## 小结与下一章

下一章：[`04-join-point-cookbook`](../part-04-join-points/01-join-point-cookbook.md)
...
上一章：[02-ltw-basics](../part-02-ltw/01-ltw-basics.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[04-join-point-cookbook](../part-04-join-points/01-join-point-cookbook.md)
