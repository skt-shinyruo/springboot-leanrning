...
- `@Configuration(proxyBeanMethods=false)`
  ...
  - 结果：可能 new 出“额外对象”，即使容器里的对应 bean 依然是 singleton。
...
这也是 Spring Boot / 自动配置里常见的写法：性能更好、语义更清晰。
...
把依赖写在 `@Bean` 方法参数上，最大的收益不是“表面上更优雅”，而是它天然满足两个可验证的事实：
...
- **误区 3：误把它当成“scope 语义变化”**
  ...
  - 经验法则：只要看到“配置类内部互调 @Bean 方法”，就默认它是风险点，优先改成“参数注入”
