...
!!! summary "章节入口"
    ...
    观察对象：Spring Boot 自动装配如何影响 Bean（Auto-configuration）。
...
**断点入口**：`ConditionEvaluator#shouldSkip`

## 自动装配的入口：`@SpringBootApplication` / `@EnableAutoConfiguration`
...
而“导入一组类”的技术手段，与 [02 章](ioc-bean-registration.md) 的 `@Import` 思想一致。
...
> 落点：把“自动装配”从概念落到方法级证据链。无需记全，只要记住 4 个稳定锚点：导入（selectImports）→ 条件（shouldSkip）→ 注册（registerBeanDefinition）→ 最终注入（doResolveDependency）。
...
3. 定义注册（决定“哪些 BeanDefinition 真正进入 registry”）
 ...
 调试时重点盯：beanName 冲突、是否允许 overriding、是否已有同类定义。
...
> 实战处理：当怀疑“自动配置类清单未导入”时，不宜从 `@Conditional` 开始推测；应先通过断点或条件报告验证“清单是否被导入 + 导入顺序”。

## 为什么自动配置不是“全都生效”？——条件（Conditions）
...
面试官最喜欢追问：需要把它解释成“时机问题”，而不是仅背诵“用 @ConditionalOnMissingBean”。
...
- 定义层时机（关键闭环）：`PostProcessorRegistrationDelegate#invokeBeanFactoryPostProcessors`
  - 结合自己的 registrar：`BeanDefinitionRegistryPostProcessor#postProcessBeanDefinitionRegistry`
...
具体做法放在下一章：[调试与自检](boot-debugging-and-observability.md)。
...
- 对应测试：`src/test/java/com/learning/springboot/springcorebeans/part02_boot_autoconfig/SpringCoreBeansAutoConfigurationOrderingLabTest.java`
  - 覆盖点：
    - 自动配置之间的顺序依赖：为什么某些 `@ConditionalOnBean` 会“表面上没生效”
