...
## 问题：团队内训讲义（Training Kit）：可直接用于授课的课时脚本

- 这页面向讲师：无需从头讲完 `spring-core-beans` 的所有机制，而是按课时选一条路线（**60/90/120 分钟**）。每条路线都给出：
  - 课时安排（分段与讲解目标）
  - 可运行的 Lab 入口（用断言固化现象）
  - 断点入口与观察清单（用证据链收敛结论）
...
- JDK：17
- IDE：IntelliJ IDEA（课堂默认）或 VS Code + Java 插件
- Maven：可用（课前先运行一次测试以完成依赖下载）
...
> 落点：让所有人用同一套“阶段感 + 断点入口 + 观察清单”描述问题，不再停留在“感觉像是 Spring 的锅”。
...
> 落点：让学员能把“概念”放回 `refresh()` 主线，建立可复用的时间线视角。
...
> 落点：学员能把一个真实问题压缩成“现象→阶段→关键方法→关键变量→修复策略”，并能复述成面试答案。
...
1. **学员直接运行全量测试/直接运行应用，断点命中频繁**
   - 对策：强制从 `*LabTest` 测试入口开始（噪音最少）
2. **断点打了很多，但 观察清单 没有统一**
...
## 小结：团队内训讲义（Training Kit）：可直接用于授课的课时脚本
