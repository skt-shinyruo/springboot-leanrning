...
1) 先读正文：按模块根 `README.md` 的目录阅读对应章节（并运行章节对应的 Lab/Test）。
...
- **从现象进入**：
  1) 在 [知识地图](appendix-knowledge-map.md) 找到现象行，直接拿到“章节 + 断点组 + 推荐 Lab”
  2) 先跑推荐 Lab 固化现象，再按断点组命中关键分支（必要时回到 [断点地图](guide-breakpoint-map.md) 选 C 组降噪）
...
- 模块入口与目录页：
  ...
  - `deepening-docs-root.md`（Docs TOC 深化）
- Part 00（Guide）：
  - `part-deepening-00-guide.md`
- Part 01（IoC Container）：
  - `part-deepening-01-ioc-container.md`
- Part 02（Boot Auto-Config）：
  - `part-deepening-02-boot-autoconfig.md`
- Part 03（Container Internals）：
  - `part-deepening-03-container-internals.md`
- Part 04（Wiring & Boundaries）：
  - `part-deepening-04-wiring-and-boundaries.md`
- Part 05（AOT & Real World）：
  - `part-deepening-05-aot-and-real-world.md`
- Appendix（工具型章节）：
