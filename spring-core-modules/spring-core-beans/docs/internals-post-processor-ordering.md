...
> - 多切面/多代理叠加与顺序（两套顺序分流）：见 [spring-core-aop：multi-proxy stacking](../../spring-core-aop/docs/proxy-stacking-multi-proxy-stacking.md)（为什么要跳：当看到“多层 proxy（套娃）”时，要把“外层/内层是谁包的（BPP）”与“链条谁先执行（advisor）”拆开；验证什么：跑对应的 multi-proxy 用例，观察 proxy 叠加与 advisor 顺序是两条不同维度）
>
> 对应可运行闭环：
>
