...
上一章：[04. 实例化前短路：postProcessBeforeInstantiation 能让构造器根本不执行](04-pre-instantiation-short-circuit.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[06. 生命周期回调顺序：Aware / BPP / init / destroy（以及 prototype 为什么不销毁）](06-lifecycle-callback-order.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
若希望同时验证“带着隐患继续运行 vs fail-fast”的差异：
...
上一章（[09. 循环依赖](../part-01-ioc-container/08-circular-dependencies.md)）读者已经建立了一个关键事实：
...
- **功能绕过**：依赖方获取到 raw，调用链绕过事务/安全/缓存等代理增强（见 [31. 代理/替换阶段：`BeanPostProcessor` 如何把 Bean “换成 Proxy”](../part-04-wiring-and-boundaries/14-proxying-phase-bpp-wraps-bean.md)）
...
- 默认：宁可失败，也不应让系统带着“绕过代理”的隐患继续运行
- 可以强行允许，但那是读者接受风险，不是 Spring 的回退机制

---

## 3. 源码最短路径：把三件事串成一条证据链

这一段的目标不是逐行复现源码，而是为读者提供一条“可在调试器中完成验证”的最短路径（完成一次验证即可形成可复用的证据链）。
...
- `allowEarlyReference`（这一个值经常决定读者有没有机会获取到 early）
...
> `getEarlyBeanReference` 解决的是“尽量让 early == final”；`allowRawInjectionDespiteWrapping` 解决的是“如果做不到一致，是否接受在存在隐患的情况下继续运行”。
...
3) **“仅引入 AOP 后开始出现循环依赖相关异常”**
   - 代理改变了 final 形态，使 early/raw 与 final/proxy 的不一致暴露出来；这不是 AOP “制造了 bug”，而是它让读者观察到了之前已经存在的边界。
...
上一章：[15. 实例化前短路：还没 new 就获取到对象了？](04-pre-instantiation-short-circuit.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[17. 生命周期回调顺序：Aware/@PostConstruct/afterPropertiesSet/initMethod](06-lifecycle-callback-order.md)
