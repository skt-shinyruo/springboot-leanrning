...
- 阅读方式建议：先跑一遍“early proxy”与“raw injection despite wrapping”两个实验，把“对象形态不一致”的坑变成可复现断言，再回到主线对照源码把证据链走通。

!!! summary "本章要点"

    - `getEarlyBeanReference` 解决的不是“能不能拿到引用”，而是：**循环依赖窗口期拿到的 early reference 是否等于最终暴露形态（proxy/wrapper）**。
    - 只懂“三级缓存救 setter 环”还不够：一旦 AOP/代理介入，如果 early 是 raw、final 是 proxy，你要么 **fail-fast**，要么 **带着隐患启动（绕过代理）**。
    - 两个典型失败形态你必须见过一次：
...
上一章（[09. 循环依赖](../part-01-ioc-container/09-circular-dependencies.md)）你已经建立了一个关键事实：
...
如果 final 阶段会被代理（典型是 AOP），那你就不能让依赖方先注入 raw，否则注入方会绕过代理。
...
- 你可以强行允许，但那是你接受风险，不是 Spring 兜底

---

## 3. 源码最短路径：把三件事串成一条证据链

这一段的目标不是逐行复现源码，而是给你一个“能在调试器里跑通”的最短路径（你跑一次，就能把这章彻底记牢）。
...
4) 你的 `SmartInstantiationAwareBeanPostProcessor#getEarlyBeanReference`（在 Lab 里实现的那个）：观察 early proxy 的产生与缓存
...
- `allowEarlyReference`（这一个值经常决定你有没有机会拿到 early）
...
- 你按接口注入：OK（proxy 满足接口类型）
- 你按实现类注入：fail（proxy 不是实现类实例）

这个失败在循环依赖里会更“早暴露”，因为 early 阶段你可能就已经拿到了 proxy 形态。
