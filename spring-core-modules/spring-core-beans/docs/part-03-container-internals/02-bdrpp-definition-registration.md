# 02. BeanDefinitionRegistryPostProcessor：在“注册阶段”动态加定义
<!-- CHAPTER-CARD:START -->
!!! summary "章节学习卡片（五问闭环）"
    - 使用方式：可先运行本章推荐 Lab，把现象固化为断言，再对照正文理解机制；真实项目里优先按“定义层/实例层/最终暴露对象”分层，再用断点与 watch list 收敛原因。

    本章围绕13. BeanDefinitionRegistryPostProcessor：在“注册阶段”动态加定义展开，主线可以概括为：`ApplicationContext#refresh` 主线：注册 BeanDefinition → BFPP 加工定义 → 实例化/注入 → BPP 增强（代理/回调）→ 生命周期与销毁。
...
上一章：[01. 容器启动与基础设施处理器：为什么注解能工作？](01-container-bootstrap-and-infrastructure.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[03. 顺序（Ordering）：PriorityOrdered / Ordered / 无序](03-post-processor-ordering.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
!!! tip "继续加深：把本章跑成可验证路线"
    ...
    建议 先跑 `SpringCoreBeansRegistryPostProcessorLabTest`，再用 `SpringCoreBeansRegistryPostProcessorLabTest.beanDefinitionRegistryPostProcessor_canRegisterNewBeanDefinitions()` 做对照；把两次差异对齐到正文的关键分支解释。
...
- **实例层（非本章）**：Bean 有了，但注入不对/变成 proxy/生命周期回调顺序奇怪
  ...
  - 对应章节：[14](03-post-processor-ordering.md)、[15](04-pre-instantiation-short-circuit.md)、[31](../part-04-wiring-and-boundaries/14-proxying-phase-bpp-wraps-bean.md)
## 源码最短路径（call chain）
...
- 若要追“最初的定义入口”（扫描/`@Bean`/`@Import`/registrar），先回到 [02](../part-01-ioc-container/01-bean-registration.md)
- `registerBeanPostProcessors` / `preInstantiateSingletons` / `doCreateBean`
  那读者处理的是“实例如何被创建/被包装”问题（见 [14](03-post-processor-ordering.md)、[25](../part-04-wiring-and-boundaries/08-programmatic-bpp-registration.md)、[31](../part-04-wiring-and-boundaries/14-proxying-phase-bpp-wraps-bean.md)）
...
- “在 post-processor 阶段 `getBean()` 引发奇怪顺序/代理缺失” → **优先实例层的时机问题**：读者可能触发了过早实例化，导致后续 BPP 来不及介入（对照 [14](03-post-processor-ordering.md)、[25](../part-04-wiring-and-boundaries/08-programmatic-bpp-registration.md)）
...
上一章：[12. 容器启动与基础设施处理器：为什么注解能工作](01-container-bootstrap-and-infrastructure.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[14. 顺序（Ordering）：PriorityOrdered / Ordered / 无序](03-post-processor-ordering.md)
