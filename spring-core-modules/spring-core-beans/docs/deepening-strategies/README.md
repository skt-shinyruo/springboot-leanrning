...
- **从现象进入**：
  1) 在 [知识地图](../appendix/92-knowledge-map.md) 找到现象行，直接拿到“章节 + 断点组 + 推荐 Lab”
  2) 先跑推荐 Lab 固化现象，再按断点组命中关键分支（必要时回到 [断点地图](../part-00-guide/013-02-breakpoint-map.md) 选 C 组降噪）
  3) 回到章节正文（章节卡片 + 继续加深提示块），把观察结果收敛为结论/反例/排错路径
- **从断点进入**：
  1) 在 [断点地图](../part-00-guide/013-02-breakpoint-map.md) 选一组断点（C1–C7）先把阶段定位清楚
  2) 再用章节末尾的 `### 对应 Lab/Test` 入口把同类场景跑出来（避免只停留在“看到了调用栈”）
  3) 回到本目录策略文件，把断点观察收敛为“关键分支 + 最短证据链 + 边界对照”（按该章需要选择性补强）

## 落地示例（把“策略”变成“正文内容”）

以 [02. Bean 注册入口：扫描、@Bean、@Import、registrar](../part-01-ioc-container/02-bean-registration.md) 为例，“继续深化”更像是把读者的下一步动作写清楚，而不是补一套固定小标题：
