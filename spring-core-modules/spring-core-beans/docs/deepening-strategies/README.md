...
- A（证据链）：在正文中明确“最终注册落点”和最短调用链（例如 registry 写入点与配置类解析点），并把它写成可断言的结论（运行哪个 Lab、看哪个变量能证明）。
- B（边界反例）：补 1–2 个“看似是注册问题但根因并不在注册”的反例（例如定义已注册但注入失败往往属于候选收敛问题；或实例层 registerSingleton 绕过 BPP 导致注解不生效）。
...
1) **10 分钟**：单独运行本章推荐 Lab/Test，保证现象与断言稳定。
2) **30 分钟**：命中 3–5 个稳定锚点断点，并用 watch list 观察到关键变量变化（能解释“为什么是这样”）。
