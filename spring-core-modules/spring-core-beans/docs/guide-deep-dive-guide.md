# 深入分析指南：将“Bean 三层模型”落实到源码与断点
<!-- CHAPTER-CARD:START -->
!!! summary "章节入口"
    - 使用方式：先运行本章给出的最小实验，以断言固定现象；随后沿“现象 → 分层（定义/候选/实例）→ 首要断点 → 观察点”的路径，形成可复核的证据链。

    观察对象：深入分析指南：将“Bean 三层模型”落实到源码与断点。
    主线位置：以 `ApplicationContext#refresh` 为时间线骨架，将 Bean 的行为分解为“定义（BeanDefinition）/实例对象（bean instance）/最终暴露对象（exposed object）”三个层次，并在关键扩展点（BDRPP/BFPP/BPP）处观察定义与实例如何被加工或替换。
...
深入源码前，先把现象归到正确层次：定义层、候选层、实例层或最终暴露对象层。层次判断清楚后，再选择第一断点和观察变量，避免在 `refresh()` 与 `doCreateBean()` 的长调用链里无目的单步。

最短入口是 `SpringCoreBeansAutowireCandidateSelectionLabTest`。先用它把候选收敛跑成断言，再按本页的“现象 → 分层 → 第一断点 → 观察清单”继续扩展到代理、循环依赖和生命周期问题。

## 从现象到断点：三层模型怎么落地

本页是源码级深入分析方法论的入口。当需要解释“为什么注入的是它”“为什么变成 proxy”“为什么循环依赖只在某些条件下能闭合”时，可以按本页的表格与断点抓手组织证据链。

若只想尽快获得一个可回归的事实入口，先运行 `SpringCoreBeansAutowireCandidateSelectionLabTest`，再回到本页按“现象 → 分层 → 第一断点 → 观察清单”推进。

- 官方文档对照（适用版本：Spring Framework 6.2.x；本仓库基线：6.2.15）：https://docs.spring.io/spring-framework/reference/core/beans.html

本章不重复 Spring 的概念性介绍，而是给出一套可复用的源码级分析方法：面对“注入为何选择某个候选”“代理为何出现或缺失”“循环依赖为何仅在特定条件下可闭合”等问题时，先用最小实验固定现象，再借助断点与观察点形成可复核的结论。

阅读路径是：先完成一个最小实验，确保入口与断言可复现；再使用症状驱动导航表选择章节与断点；必要时对照主线章的时间线，将观察点放回正确阶段。

!!! example "本章配套实验（先运行）"

    - 核心入口：`SpringCoreBeansAutowireCandidateSelectionLabTest` / `SpringCoreBeansContainerLabTest` / `SpringCoreBeansBeanCreationTraceLabTest`
...
为避免阅读仅停留在主观理解层面，本章给出 10/30/3 的自检标准。它不强调背诵，而强调可验证与可复现：

- **10 分钟（可复述）**：能够说明 Bean 的“三层对象”与“最终暴露对象”分别指代什么（Definition / Instance / Exposed），并给出一个反例解释它们可能不一致（例如代理、FactoryBean）。
- **30 分钟（可验证）**：完成一次方法级实验/测试运行，并在断点中验证“对象在哪一层被替换/为何被替换”（而非仅依赖日志推断）。
...
若上述三项目标均可完成，可认为已达到本章验证目标；若尚未完成，先按本章实践路线完成最小实验，再进入后续章节，以避免在源码细节中失去主线。
...
- [从 `refresh()` 到 `doCreateBean()`：把 Spring Bean “变成对象”的主线走通（源码级）](internals-refresh-to-bean-creation-mainline.md)

在首次进行断点与源码分析时，较常见的两类困难如下：

- **范围过大（信息噪声较高）**：直接执行 `spring-boot:run` 或运行全量测试，通常会显著增加断点命中次数并放大噪声
  - 处理路径：优先使用本仓库 `*LabTest`，将问题限定为“最小容器 + 最小现象 + 最小断言”
- **缺少分层（对象形态不清晰）**：将“定义层/候选层/实例层”混合观察，容易在长调用栈中失去主线
  - 处理路径：先明确当前关注对象属于哪一层：**BeanDefinition**（是否已注册/已合并）、**候选集合**（候选范围与收敛过程）、**最终暴露对象**（为何呈现为 proxy 等形态）

### 最小抓手：5 类对象与关键链路（将概念转化为可观察对象）
...
- **调试成本较高**
  ...
  - 处理路径：优先使用条件断点（以 `beanName` 过滤）→ 固定监视列表（观察清单）→ 避免以单步方式贯穿全链路

对首次进入本模块的读者，先完成可复现的最小实验，再围绕关键入口逐步加深：

1. **首先（约 30 分钟）完成 3 个最小实验**：命令、断点与观察点均已给出
   - 见：[30 分钟快速闭环：先快后深（3 个最小实验入口）](guide-quickstart-30min.md)
...
- [断点地图：容器主线可复用断点/观察点清单](guide-breakpoint-map.md)

### 第一轮阅读：以较低成本把握阶段边界
> 官方参考（Spring Framework 6.2.x，BeanFactory/Bean 语义总览）：https://docs.spring.io/spring-framework/reference/core/beans.html


第一轮阅读的目标在于把握阶段边界，而非追求完整细节。按以下顺序推进：

1. 先完成 [30 分钟快速闭环：先快后深（3 个最小实验入口）](guide-quickstart-30min.md)，确保能够通过断点验证“候选收敛 / prototype 注入边界 / proxy 替换”三类现象
...
### 方法验证：`SpringCoreBeansAutowireCandidateSelectionLabTest` 的 3 条关键结论

完成该 Lab 后，至少要能复述并验证以下 3 条结论（每条均可定位到方法级入口）：
...
在断点中固定监视列表（观察清单），以避免在每次调试时重复定位变量：
...
本章以最小可复现实验为单位推进。每个实验需要能回答三个问题：现象是什么、断点设置在哪里、证据是什么。
...
3. BeanDefinition 从哪里来（列 3 条入口，并说明落点）
   - 入口：阅读 [02](ioc-bean-registration.md) + 运行 `SpringCoreBeansBootstrapInternalsLabTest`
...
- 能够通过 `*LabTest` 将问题限定为方法级最小复现，并给出下一步入口（章节入口与断点入口）
...
- refresh 主线与创建主线：[`refresh()` → `doCreateBean()`](internals-refresh-to-bean-creation-mainline.md)
- 断点可复用清单：[`断点地图`](guide-breakpoint-map.md)
...
3. **最后给证据链**：点名 1–2 个关键方法 + 3 个监视列表（观察清单）要点 + 1 个可运行的 Lab。

可将上述结构与 `appendix-interview-playbook.md` 的“标准结构”配合使用，并沿“现象 → 章节 → 断点 → Lab”的路径完成验证。

## 验收口径：能否把现象压成证据链

需要用 2–3 句话向他人解释：

1. 为什么本模块先运行 Lab，再阅读正文？
2. 当遇到注入失败/代理不生效/循环依赖时，如何沿“现象 → 章节 → 断点 → Lab”的路径完成一次可验证的分析？
3. 下一次学习/排障的入口应从何处进入（Knowledge Map / 断点包 / Branch Matrix）？
