...
本页保留为“源码级深入分析方法论”的入口页：当需要解释“为什么注入的是它/为什么变成 proxy/为什么循环依赖只在某些条件下能闭合”时，可以按本页的表格与断点抓手组织证据链。
...
- 阅读路径：先完成一个最小实验（确保入口与断言可复现），再使用“症状驱动导航表”选择章节与断点；必要时对照主线章的时间线，将观察点放回正确阶段。
