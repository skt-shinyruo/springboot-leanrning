...
- 若的目标是面试：建议先刷 `appendix-interview-playbook.md` 的题目结构，再回到本知识地图用“现象→章节→断点→Lab”把每个答案补齐证据链。
...
| 需要“从异常到断点入口”的方法论 | [Debugging and Observability](boot-debugging-and-observability.md)、[Debugger Pack](appendix-debugger-pack.md) | 见 Debugger Pack | 见 [断点地图](guide-breakpoint-map.md#c1) | `SpringCoreBeansBreakpointPackLabTest` |
...
!!! tip "继续加深：把本章跑成可验证路线"
    ...
    - 下一跳：需要可复用断点组时，回到 [断点地图](guide-breakpoint-map.md)；需要把现象分型成最短 SOP 时，回到 [生产排障清单](appendix-production-troubleshooting-checklist.md)。
<!-- AE-DEEPENING:END -->
