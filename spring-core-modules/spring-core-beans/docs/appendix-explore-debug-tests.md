...
## 起点：Explore/Debug 用例（可选启用，不影响默认回归）
...
先运行 `SpringCoreBeansSingletonCacheExploreTest`（三层缓存最直观），再按本页的断点与观察清单 扩展到其它 Explore 用例。
...
## 验证标准：Explore/Debug 用例（可选启用，不影响默认回归）
...
## 收束：Explore/Debug 用例（可选启用，不影响默认回归）
