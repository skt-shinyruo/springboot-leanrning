...
## 问题：同名 BeanDefinition 是覆盖，还是直接失败

同名注册不是注入歧义，而是定义层冲突。容器到底允许后注册覆盖前注册，还是遇到同名就失败，取决于 `allowBeanDefinitionOverriding` 这一个开关。

先运行 `SpringCoreBeansBeanDefinitionOverridingLabTest`，把核心现象固定为可复现事实；随后围绕入口方法、关键分支和可观察变量阅读正文。
...
## 验收口径：BeanDefinition 覆盖（overriding）
...
## 小结：BeanDefinition 覆盖（overriding）
