...
`FactoryBean` 是 Spring 里很有“容器味”的机制：
...
## isSingleton 的语义：容器是否缓存“product”
...
- early reference 阶段若需要 product，确保 **early == final** 形态一致
...
- “按类型发现/条件装配行为很怪” → **定义层（类型元数据）**：检查 `getObjectType()` 是否可靠（见 [29](wiring-factorybean-edge-cases.md)）
...
- 能返回明确类型就返回明确类型
- 如果确实无法确定，至少在文档/注释中说明原因，并配套测试覆盖边界（本模块已提供，见 [FactoryBean 边界：getObjectType 返回 null 会让“按类型发现”失效](wiring-factorybean-edge-cases.md) + Lab）
...
- 需要解释清楚：为什么 `&beanName` 可以获取到 factory 自己吗？
- 需要解释清楚：`isSingleton()` 控制的是“product 是否缓存”而不是“factory 是否单例”吗？
...
## 误判点：FactoryBean 深潜：product vs factory、类型匹配
