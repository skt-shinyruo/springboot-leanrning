...
上一章：[04. 面试复述模板（Interview Playbook）：用“证据链”回答 Spring IoC](04-interview-playbook.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[06. <!--](06-spring-beans-public-api-index.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
几乎所有 IoC 相关事故，都能归到 refresh 的某一段（见 [10. 主线时间线](../part-00-guide/02-mainline-timeline.md)）：
...
若需要更系统的“现象 → 章节 → 断点组 → Lab”（学习/复盘视角）索引，建议跳到：[知识地图](03-knowledge-map.md)。

| 现象（Symptoms） | 首要阶段 | 第一断点（最短证据链） | 第一章（最短阅读） | 推荐 Lab |
| --- | --- | --- | --- | --- |
| 扫不到/导不进（NoSuchBeanDefinition） | 定义层 | `DefaultListableBeanFactory#registerBeanDefinition` | `part-01-ioc-container/01-bean-registration.md` | `SpringCoreBeansComponentScanLabTest` / `SpringCoreBeansImportLabTest` |
| 多候选歧义（NoUniqueBeanDefinition） | 注入解析 | `DefaultListableBeanFactory#doResolveDependency` | `part-01-ioc-container/02-dependency-injection-resolution.md` | `SpringCoreBeansAutowireCandidateSelectionLabTest` |
| 代理不生效（像绕过 AOP） | 创建/after-init | `applyBeanPostProcessorsAfterInitialization` | `part-04-wiring-and-boundaries/14-proxying-phase-bpp-wraps-bean.md` | `SpringCoreBeansProxyingPhaseLabTest`（或对应分支矩阵） |
| 循环依赖异常/行为诡异 | 创建层（窗口期） | `DefaultSingletonBeanRegistry#getSingleton` | `part-01-ioc-container/08-circular-dependencies.md` | `SpringCoreBeansCircularDependencyBoundaryLabTest` |
| `@Value` 值不对/缺失不失败 | 定义层 + 注入阶段 | `AbstractBeanFactory#resolveEmbeddedValue` | `part-04-wiring-and-boundaries/17-value-placeholder-resolution-strict-vs-non-strict.md` | `SpringCoreBeansValuePlaceholderResolutionLabTest` |
| `FactoryBean` 获取到的不是容易误以为的对象 | getBean 分流 | `AbstractBeanFactory#doGetBean` | `part-01-ioc-container/07-factorybean.md` | `SpringCoreBeansFactoryBeanDeepDiveLabTest` |
| Boot 自动装配“偶发失效” | 定义层顺序 | `AutoConfigurationImportSelector#selectImports` | `part-02-boot-autoconfig/02-auto-config-ordering.md` | `SpringCoreBeansAutoConfigurationOrderingLabTest` |
| XML/namespace 解析失败 | 定义层输入 | `XmlBeanDefinitionReader#loadBeanDefinitions` | `part-05-aot-and-real-world/03-xml-bean-definition-reader.md` | `SpringCoreBeansXmlBeanDefinitionReaderLabTest` |
| AOT/Native 行为缺失（反射/资源） | 构建期契约 | `RuntimeHintsRegistrar#registerHints` | `part-05-aot-and-real-world/02-runtimehints-basics.md` | `SpringCoreBeansAotRuntimeHintsLabTest` |
...
2) 第一断点：`DefaultListableBeanFactory#doResolveDependency`（断点地图：[C6](../part-00-guide/07-breakpoint-map.md#c6)）
3) 三个观察点（最小够用）：
...
2) 第一断点：
   - `PostProcessorRegistrationDelegate#registerBeanPostProcessors`（断点地图：[C4](../part-00-guide/07-breakpoint-map.md#c4)）
   - `AbstractAutowireCapableBeanFactory#applyBeanPostProcessorsAfterInitialization`（断点地图：[C7](../part-00-guide/07-breakpoint-map.md#c7)）
3) 三个观察点（最小够用）：
...
2) 第一断点：`DefaultSingletonBeanRegistry#getSingleton`（断点地图：[C5](../part-00-guide/07-breakpoint-map.md#c5)）
3) 三个观察点（最小够用）：
...
上一章：[93. 面试复述模板（Interview Playbook）](04-interview-playbook.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[95. spring-beans Public API Index（索引）](06-spring-beans-public-api-index.md)
