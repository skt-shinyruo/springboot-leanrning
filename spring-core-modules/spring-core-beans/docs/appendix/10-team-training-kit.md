...
上一章：[09. Debugger Pack（断点包总入口）](09-debugger-pack.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[11. 自测题：是否能够真的理解了？](11-self-check.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
!!! example "讲师可先运行的入口（上课前自检）"
    ...
    - 30 分钟快启（课堂最常用的 3 个实验入口）：`part-00-guide/05-quickstart-30min.md`
    - 知识地图（遇到问题直接定位章节/断点/Lab）：`appendix/03-knowledge-map.md`
...
- 入口：`part-00-guide/05-quickstart-30min.md`
...
参考：`appendix/04-interview-playbook.md`

## 2. 90 分钟进阶（适合“源码进阶 + 排障方法”）
...
1) 主线时间线（阶段感）：`part-00-guide/02-mainline-timeline.md`
2) refresh 调用链（锚点）：`part-00-guide/06-applicationcontext-refresh-call-chain.md`
3) 断点地图（收敛）：`part-00-guide/07-breakpoint-map.md`
...
- `appendix/03-knowledge-map.md`（按现象选题：注入失败/代理/循环依赖/占位符/转换）
...
- 章节补齐/排障固化：`appendix/01-common-pitfalls.md`
- Debugger Pack：`appendix/09-debugger-pack.md`
- 自测题：`appendix/11-self-check.md`
- 深入分析指南：`part-00-guide/03-deep-dive-guide.md`
...
2) 按 `appendix/04-interview-playbook.md` 的结构写出标准答案（包含方法级证据链）
...
上一章：[98. Debugger Pack（断点包总入口）](09-debugger-pack.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[99. 自测题：是否能够真的理解了？](11-self-check.md)
