...
你可以用 Debugger Pack 给自己做一个非常明确的训练闭环（适用于源码进阶/团队内训/面试）：

1) **10 分钟（可跑）**：跑通 `SpringCoreBeansBreakpointPackLabTest`，确保环境与入口 OK。
2) **30 分钟（可看见）**：只用本章 2.x 的断点 + 本章 3 的 watch list，把主线数据结构变化看见。
3) **3 分钟（可复述）**：把你看到的现象复述成“结论 → 证据链（方法名）→ 反例/坑”，对标：`appendix/93-interview-playbook.md`。
...
- 你的目标不是背“名词”，而是能在面试里说清楚：**发生在哪个阶段（refresh 哪一段）/ 证据链怎么证明 / 典型坑是什么**。
...
Debugger Pack 的目的不是“列断点”，而是帮你形成一种稳定输出：
...
如果你能在 3 分钟内按这个卡片说完一个问题，你就具备“源码进阶/面试/排障”三合一的能力闭环。
