...
!!! tip "内容级再加深（A–E 维度）"
    ...
    - A（证据链）：为每类症状补“第一断点入口 + 关键变量”，把清单变成可执行 SOP。
    - B（边界反例）：“误判对照”：相似症状可能属于不同机制域，如何快速分流。
    - C（排障 SOP）：把排障清单与 docs/README 症状导航打通（双向链接）。
    - D（断点观察）：与 Debugger Pack 互链：每类症状给推荐断点组。
    - E（面试复述）：把排障题转成面试追问（如何定位/如何证明/如何修复）。
<!-- AE-DEEPENING:END -->
