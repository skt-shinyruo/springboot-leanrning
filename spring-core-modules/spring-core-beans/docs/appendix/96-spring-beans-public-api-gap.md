...
当你在某个 API/机制域上出现 Gap，最容易走偏的方式是“看源码看到迷路”。更稳的套路是先把 **最短调用链** 固定下来：
...
你不需要把链路写成长篇大论；但必须能做到“方法级可指认”。

## 排障分流（Gap 视角：你到底缺的是哪一段）

| 现象 | 更像缺口在哪 | 建议先补哪类材料 |
| --- | --- | --- |
| 你知道结论，但一问“哪个方法证明”就卡住 | 调用链未固化 | 先补：章节里的“源码调用链（方法级）”与断点入口 |
| 你会下断点，但不知道看哪些变量 | 可观测闭环不完整 | 先补：watch list（最小够用版）与条件断点模板 |
| 你能解释主线，但遇到边界就崩 | 边界用例缺失 | 先补：一个最小 Lab 把边界固化成断言 |
| 你能复现，但不知道怎么修 | 诊断→修复路径缺失 | 先补：排障决策表（Symptoms→Evidence→Fix→Verify） |
