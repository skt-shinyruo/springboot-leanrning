...
**断点入口**：`DefaultListableBeanFactory#addBeanPostProcessor` / `PostProcessorRegistrationDelegate#registerBeanPostProcessors`

## 现象 1：手工添加的 BPP 会比“作为 bean 自动发现”的 BPP 更早执行
...
## 现象 2：programmatic BPP 的执行顺序 = 注册顺序（不是 Ordered 顺序）
...
原因也很直接：`addBeanPostProcessor` 的语义就是维护一个 list（概念级伪代码足够解释现象）：
...
> 落点：排障时用最短路径确认：当前 BPP 顺序为什么是这样？Ordered 到底有没有机会生效？
...
3. 手工 `addBeanPostProcessor(...)`：直接修改最终 list
  - **绕过分组与排序输入**，因此不能期待 `Ordered/@Order` 在“追加式注册”场景自动生效
...
- 修复方式（工程化）：
  - 要么回到定义层注册（默认路径），要么显式补齐：`autowireBean` + `initializeBean`（但要理解副作用与边界）。
- 最小复现：
