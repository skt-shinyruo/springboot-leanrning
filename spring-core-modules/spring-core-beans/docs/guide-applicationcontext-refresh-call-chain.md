...
## 读法：把 `refresh()` 当作排障坐标系

`refresh()` 不是一个需要死记的长方法，而是容器排障的坐标系。读者应先知道当前问题落在“改定义、注册 BPP、创建单例”哪一段，再决定是否继续下钻到 `doCreateBean()`、依赖解析或三层缓存。

本页的阅读顺序是：先跑 `SpringCoreBeansContainerLabTest`，再看三段关键阶段，最后把异常映射到阶段和第一断点。

## `refresh()` 主线：先定位阶段，再进入分支

- 落点：把“知道 refresh 是主线入口”推进到“能解释 refresh 的阶段，并能将断点设置在正确的阶段入口”。
...
无需一次性背完所有步骤，但必须能把关键阶段按顺序复述出来。只有阶段顺序清楚，断点才打得准。

> 提示：本仓库的断点地图会把下面的关键节点转成“可复用断点清单”，见：[02：断点地图](guide-breakpoint-map.md)。
