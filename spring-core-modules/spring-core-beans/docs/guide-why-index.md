...
本页收集的是 Spring Beans 中最容易被问到、也最容易误答的“为什么”。每个条目都按同一条证据链组织：结论是什么、为什么重要、用哪个 Lab 和断点验证、常见误判在哪里。

读者不需要顺序背完整页。遇到三级缓存、early reference、proxy 替换或 self-invocation 等问题时，先定位对应 Why，再按该条目的实验和断点完成验证。
...
这页是一个高频问题索引：每个 Why 都把一个常见困惑压缩成三样东西：
...
因此，本页固定采用“结论 → 影响范围 → 10 分钟证据链 → 误判对照 → 下一步去哪读”的结构。
