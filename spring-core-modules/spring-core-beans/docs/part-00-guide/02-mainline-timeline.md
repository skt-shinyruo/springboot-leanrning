# 02. 主线时间线：IoC 容器从 refresh 到创建 Bean
<!-- CHAPTER-CARD:START -->
!!! summary "章节学习卡片（五问闭环）"
    - 使用方式：可先运行本章推荐 Lab，把主线/断点闭环完成验证，再回到正文按“时间线/分支矩阵/证据链”定位机制窗口；最后用自检题把表达固化成可复述答案。

    本章围绕主线时间线：IoC 容器从 refresh 到创建 Bean展开，主线可以概括为：`ApplicationContext#refresh` 主线：注册 BeanDefinition → BFPP 加工定义 → 实例化/注入 → BPP 增强（代理/回调）→ 生命周期与销毁。
...
上一章：[03. 深入分析指南：将“Bean 三层模型”落实到源码与断点](03-deep-dive-guide.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[06. `refresh()` 调用链（容器从“定义”到“实例”的主线）](06-applicationcontext-refresh-call-chain.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
- `part-03-container-internals/02-bdrpp-definition-registration.md`
...
- `part-04-wiring-and-boundaries/09-smart-initializing-singleton.md`
...
推荐复习入口：`appendix/04-interview-playbook.md`（Q1/Q5 等题型都以时间线为骨架）。
...
!!! tip "继续加深：把本章跑成可验证路线"
    ...
    建议 先跑 `SpringCoreBeansMainlineCallChainLabTest`，再用 `SpringCoreBeansBreakpointPackLabTest` 做对照；把两次差异对齐到正文的关键分支解释。
...
下一章见：[第 13 章：01. `ApplicationContext#refresh` 调用链（主线）](06-applicationcontext-refresh-call-chain.md)


<!-- BOOKIFY:START -->

上一章：[第 9 章：00 - Deep Dive Guide（spring-core-beans）](03-deep-dive-guide.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[第 13 章：01. `ApplicationContext#refresh` 调用链（主线）](06-applicationcontext-refresh-call-chain.md)
