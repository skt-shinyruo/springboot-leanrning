...
上一章：[02. 主线时间线：IoC 容器从 refresh 到创建 Bean](02-mainline-timeline.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[05. 30 分钟快速闭环：先快后深（3 个最小实验入口）](05-quickstart-30min.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
- [从 `refresh()` 到 `doCreateBean()`：把 Spring Bean “变成对象”的主线走通（源码级）](../part-03-container-internals/07-refresh-to-bean-creation-mainline.md)
...
1) **首先（约 30 分钟）完成 3 个最小实验**：命令、断点与观察点均已给出
   - 见：[第 12 章：01. 30 分钟快速闭环：先快后深（3 个最小实验入口）](05-quickstart-30min.md)
...
- [第 13 章：02. 断点地图：容器主线可复用断点/观察点清单](07-breakpoint-map.md)
...
|---|---|---|---|---|
| Bean 未注册（扫描/导入/`@Bean` 未生效） | 定义层（BeanDefinition 图未形成） | [18. refresh→doCreateBean 主线](../part-03-container-internals/07-refresh-to-bean-creation-mainline.md)<br>[12. 注解为何生效（bootstrap）](../part-03-container-internals/01-container-bootstrap-and-infrastructure.md) | `PostProcessorRegistrationDelegate#invokeBeanFactoryPostProcessors`<br>`ConfigurationClassPostProcessor#processConfigBeanDefinitions` | `SpringCoreBeansRegistryPostProcessorLabTest`<br>`SpringCoreBeansBootstrapInternalsLabTest` |
| `@Autowired` 为 null / `@PostConstruct` 未执行（常见于手工启动 BeanFactory） | 实例层（规则未装入：BPP 缺失） | [12. 注解为何生效（bootstrap）](../part-03-container-internals/01-container-bootstrap-and-infrastructure.md) | `PostProcessorRegistrationDelegate#registerBeanPostProcessors`<br>`AutowiredAnnotationBeanPostProcessor#postProcessProperties`<br>`InitDestroyAnnotationBeanPostProcessor#postProcessBeforeInitialization` | `SpringCoreBeansBootstrapInternalsLabTest`<br>`SpringCoreBeansBeanFactoryApiLabTest` |
| 启动即异常 vs 第一次 `getBean` 才异常 | refresh 预实例化 vs on-demand | [18. refresh→doCreateBean 主线](../part-03-container-internals/07-refresh-to-bean-creation-mainline.md) | `DefaultListableBeanFactory#preInstantiateSingletons`<br>`AbstractBeanFactory#doGetBean` | `SpringCoreBeansPreInstantiationLabTest` |
| 单个注入歧义（NoUniqueBeanDefinitionException） | 候选层（候选集合收敛失败） | [33. 候选选择与顺序](../part-04-wiring-and-boundaries/16-autowire-candidate-selection-primary-priority-order.md) | `DefaultListableBeanFactory#doResolveDependency` | `SpringCoreBeansInjectionAmbiguityLabTest`<br>`SpringCoreBeansAutowireCandidateSelectionLabTest` |
| 循环依赖：setter 在特定条件下可闭合，constructor 一般无法闭合；或 early reference 形态异常 | 实例层（创建窗口 + 缓存/early reference） | [18. refresh→doCreateBean 主线](../part-03-container-internals/07-refresh-to-bean-creation-mainline.md)<br>[16. early reference 与循环依赖](../part-03-container-internals/05-early-reference-and-circular.md) | `DefaultSingletonBeanRegistry#getSingleton/addSingletonFactory`<br>`AbstractAutowireCapableBeanFactory#doCreateBean`（earlySingletonExposure） | `SpringCoreBeansCircularDependencyBoundaryLabTest`<br>`SpringCoreBeansEarlyReferenceLabTest` |
| 代理不生效 / 自调用绕过 / 误判为“存在 AOP 但未生效” | 实例层（BPP 包装阶段） | [31. BPP 如何把 Bean 换成 Proxy](../part-04-wiring-and-boundaries/14-proxying-phase-bpp-wraps-bean.md) | `BeanPostProcessor#postProcessAfterInitialization` | `SpringCoreBeansProxyingPhaseLabTest` |
| `getBean("x")` 获取到的类型与预期不一致，`&x` 又呈现不同语义 | 实例层（FactoryBean 语义） | [08. FactoryBean：& 前缀与产品对象](../part-01-ioc-container/07-factorybean.md)<br>[18. refresh→doCreateBean 主线](../part-03-container-internals/07-refresh-to-bean-creation-mainline.md) | `AbstractBeanFactory#getObjectForBeanInstance` | `SpringCoreBeansFactoryBeanDeepDiveLabTest` |
...
将 [01. Bean 运行机制：从 BeanDefinition 到最终暴露对象](../part-01-ioc-container/09-bean-mental-model.md) 的三层结构对应到源码，最小可以按以下结构建立映射。
...
- [02. 注册入口](../part-01-ioc-container/01-bean-registration.md)
- [06. PostProcessor 概览](../part-01-ioc-container/05-post-processors.md)
- [12. 注解为何生效（基础设施处理器）](../part-03-container-internals/01-container-bootstrap-and-infrastructure.md)
- [13. BDRPP：定义注册](../part-03-container-internals/02-bdrpp-definition-registration.md)
...
- [30. 注入发生在什么时候（field vs constructor）](../part-04-wiring-and-boundaries/13-injection-phase-field-vs-constructor.md)
- [15. 实例化前短路](../part-03-container-internals/04-pre-instantiation-short-circuit.md)
- [31. BPP 如何把 Bean 换成 Proxy](../part-04-wiring-and-boundaries/14-proxying-phase-bpp-wraps-bean.md)
...
- [09. 循环依赖概览](../part-01-ioc-container/08-circular-dependencies.md)
- [16. early reference 与循环依赖：getEarlyBeanReference](../part-03-container-internals/05-early-reference-and-circular.md)
...
- [33. 候选选择与顺序](../part-04-wiring-and-boundaries/16-autowire-candidate-selection-primary-priority-order.md)
...
3) BeanDefinition 从哪里来（列 3 条入口，并说明落点）
   - 入口：阅读 [02](../part-01-ioc-container/01-bean-registration.md) + 运行 `SpringCoreBeansBootstrapInternalsLabTest`
...
- refresh 主线与创建主线：[`refresh()` → `doCreateBean()`](../part-03-container-internals/07-refresh-to-bean-creation-mainline.md)
- 断点可复用清单：[`断点地图`](07-breakpoint-map.md)
...
上一章：[第 10 章：主线时间线：Spring Core Beans（IoC 容器）](02-mainline-timeline.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[第 12 章：01. 30 分钟快速闭环：先快后深（3 个最小实验入口）](05-quickstart-30min.md)
...
建议将上述结构与 `appendix/04-interview-playbook.md` 的“标准结构”配合使用，并沿“现象 → 章节 → 断点 → Lab”的路径完成验证。
