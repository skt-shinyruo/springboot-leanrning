...
上一章：[第 11 章：00. 深挖指南：把“Bean 三层模型”落到源码与断点](011-00-deep-dive-guide.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[第 13 章：02. 断点地图（容器主线：可复用断点/观察点清单）](013-02-breakpoint-map.md)
<!-- GLOBAL-BOOK-NAV:END -->

## 导读

- 本章目标：给出一条**30 分钟可跑通、可下断点、可形成正反馈**的快启路线。
...
!!! summary "本章要点"
    ...
    3) **为什么读者拿到的是 proxy？**（换壳发生在哪一段、是谁换的）
...
- **30 分钟（能跑通并证明）**：至少跑通 1 个实验，并能用断点/观察点解释“为什么结果是这样”（不要只看输出）。
...
- 读者已经拿到了“最小闭环”：**从测试方法进、用固定断点收敛噪音、用 watch list 拿到证据**
...
- 建议：跑完 ``SpringCoreBeansLabTest#usesQualifierToResolveMultipleBeans`` 后，把上述观察点逐条对照，写出读者自己的 1–2 句结论（可复述）。
...
若跑了 `SpringCoreBeansBreakpointPackLabTest`，至少能复述以下 3 条“可断言结论”：
...
1) 跑一条最小容器（应能够看到 BeanDefinition → bean 实例的转换）
2) 跑一条 refresh 主线（应能够把“处理器/注入/回调”放回时间线）
3) 下两三个关键断点（应能够在调试器里看见关键数据结构变化）
...
3) 如何用一个断点证明“这个行为确实发生了”（比如代理替换/候选收敛/early reference）？
