...
上一章：[05. 30 分钟快速闭环：先快后深（3 个最小实验入口）](05-quickstart-30min.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[07. 断点地图（容器主线：可复用断点/观察点清单）](07-breakpoint-map.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
- **3 个链接（可继续深入分析）**：本章只提供“地图”，细节必须跳转到对应章节完成证明：
  - refresh → doCreateBean 主线：`07-refresh-to-bean-creation-mainline.md`
  - 处理器排序与两段式算法：`03-post-processor-ordering.md`
  - early reference 与循环依赖：`05-early-reference-and-circular.md`

## 主线伪代码（把 refresh 当成“时间线”读）

无需一次性背完所有步骤，但必须能把“关键阶段”按顺序复述出来（这样断点才打得准）。

> 提示：本仓库的断点地图会把下面的关键节点转成“可复用断点清单”，见：[02：断点地图](07-breakpoint-map.md)。
...
- 第 10 段（批量创建）：非 lazy 单例会在这里被创建出来（因此很多问题会在此处集中暴露）
...
因为“启动期集中暴露问题”几乎都意味着：**它是非 lazy 单例，且被预实例化触发了**。
...
进一步入口：`appendix/05-production-troubleshooting-checklist.md`（总分流表）/ `appendix/09-debugger-pack.md`（断点包）。
...
推荐复习入口：`appendix/04-interview-playbook.md`（Q1/Q5 等）。
...
上一章：[part-00-guide/01-quickstart-30min.md](05-quickstart-30min.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[part-00-guide/02-breakpoint-map.md](07-breakpoint-map.md)
