    # 容器启动与基础设施 Bean
    <!-- CHAPTER-CARD:START -->
    !!! summary "章节入口"
        - 这一页只回答：为什么注解处理器、自动装配和基础设施能够在容器里生效？
        - 最短命令：`mvn -pl :spring-core-beans -Dtest=SpringCoreBeansBootstrapInternalsLabTest test`
        - 相邻主题只做跳转，不在本页重复展开。

        观察对象：注解处理器、内部 infrastructure Bean、role 标记和 bootstrap 注册窗口。
        主线位置：容器与注册。
        对照入口：`SpringCoreBeansBootstrapInternalsLabTest` / `SpringCoreBeansInfrastructureBeanRoleLabTest`。
    <!-- CHAPTER-CARD:END -->

    ## 归属边界

    这一页只回答一个问题：为什么注解处理器、自动装配和基础设施能够在容器里生效？

    本页负责把这个问题收束到一个可运行证据入口。相邻主题只在“相邻跳转”中出现，避免同一个知识点散落到多个文件。

    ## 最短证据入口

    - `SpringCoreBeansBootstrapInternalsLabTest`
- `SpringCoreBeansInfrastructureBeanRoleLabTest`

    ## 观察口径

    | 观察点 | 看什么 | 不在这里展开 |
    | --- | --- | --- |
    | 问题定位 | 为什么注解处理器、自动装配和基础设施能够在容器里生效？ | 支持页只负责导航 |
    | 运行证据 | `SpringCoreBeansBootstrapInternalsLabTest` / `SpringCoreBeansInfrastructureBeanRoleLabTest` | 不用未验证的口头结论替代 Lab |
    | 边界判断 | 注解处理器、内部 infrastructure Bean、role 标记和 bootstrap 注册窗口。 | 相邻 owner 文档另行负责 |

    ## 相邻跳转

    - [refresh-mainline.md](refresh-mainline.md)
- [post-processors-overview.md](post-processors-overview.md)
- [appendix-knowledge-map.md](appendix-knowledge-map.md)

    ## 小结

    container-bootstrap-and-infrastructure.md 的完成标准是：读者能用上面的 Lab 证明“为什么注解处理器、自动装配和基础设施能够在容器里生效？”，并知道哪些相邻问题应该跳到其他 owner 文档。
