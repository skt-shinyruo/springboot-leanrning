    # BeanDefinition 注册：谁把定义放进容器
    <!-- CHAPTER-CARD:START -->
    !!! summary "章节入口"
        - 这一页只回答：一个 BeanDefinition 是如何被注册进容器的？
        - 最短命令：`mvn -pl :spring-core-beans -Dtest=SpringCoreBeansBeanDefinitionRegistrationDiffLabTest test`
        - 相邻主题只做跳转，不在本页重复展开。

        观察对象：扫描、配置类解析和普通定义注册如何把 BeanDefinition 放入 registry。
        主线位置：容器与注册。
        对照入口：`SpringCoreBeansBeanDefinitionRegistrationDiffLabTest` / `SpringCoreBeansComponentScanLabTest`。
    <!-- CHAPTER-CARD:END -->

    ## 归属边界

    这一页只回答一个问题：一个 BeanDefinition 是如何被注册进容器的？

    本页负责把这个问题收束到一个可运行证据入口。相邻主题只在“相邻跳转”中出现，避免同一个知识点散落到多个文件。

    ## 最短证据入口

    - `SpringCoreBeansBeanDefinitionRegistrationDiffLabTest`
- `SpringCoreBeansComponentScanLabTest`

    ## 观察口径

    | 观察点 | 看什么 | 不在这里展开 |
    | --- | --- | --- |
    | 问题定位 | 一个 BeanDefinition 是如何被注册进容器的？ | 支持页只负责导航 |
    | 运行证据 | `SpringCoreBeansBeanDefinitionRegistrationDiffLabTest` / `SpringCoreBeansComponentScanLabTest` | 不用未验证的口头结论替代 Lab |
    | 边界判断 | 扫描、配置类解析和普通定义注册如何把 BeanDefinition 放入 registry。 | 相邻 owner 文档另行负责 |

    ## 相邻跳转

    - [import-selector-and-registrar.md](import-selector-and-registrar.md)
- [programmatic-registration.md](programmatic-registration.md)
- [bean-definition-metadata-and-origin.md](bean-definition-metadata-and-origin.md)
- [appendix-knowledge-map.md](appendix-knowledge-map.md)

    ## 小结

    bean-definition-registration.md 的完成标准是：读者能用上面的 Lab 证明“一个 BeanDefinition 是如何被注册进容器的？”，并知道哪些相邻问题应该跳到其他 owner 文档。
