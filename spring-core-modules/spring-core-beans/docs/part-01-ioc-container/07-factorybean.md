...
上一章：[06. `@Configuration` 增强与 `@Bean` 语义（proxyBeanMethods）](06-configuration-enhancement.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[08. 循环依赖：现象、原因与规避（constructor vs setter）](08-circular-dependencies.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
- 常见追问：`getObjectType()` 返回 `null` / 返回错误类型 有什么误区？为什么 `allowEagerInit=false` 会放大它？
  - 答题要点：会影响 type-based 查找与条件装配（例如 `@ConditionalOnMissingBean`）；如果 `getObjectType()` “说谎”，甚至会造成候选集合被污染（找不到本该找到的 product）。需要时对照 [23](../part-04-wiring-and-boundaries/06-factorybean-deep-dive.md) 与 [29](../part-04-wiring-and-boundaries/12-factorybean-edge-cases.md) 深入分析。
...
上一章：[07. @Configuration 增强：proxyBeanMethods 与 @Bean 语义](06-configuration-enhancement.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[09. 循环依赖概览：三级缓存与现象分类](08-circular-dependencies.md)
