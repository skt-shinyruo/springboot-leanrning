...
## 问题：alias 是新 Bean，还是同一个 Bean 的另一个名字

alias 不会复制 BeanDefinition，也不会创建第二个实例。它只是把别名解析回 canonical name，因此排查“按名称找不到”或 `@Resource` name-first 行为时，必须先看 alias 解析链路。
...
## 验收口径：Bean 名称与 alias：同一个实例，多一个名字
需要解释清楚：
