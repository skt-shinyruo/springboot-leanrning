...
先运行 `SpringCoreBeansEarlyReferenceLabTest` 与 `SpringCoreBeansRawInjectionDespiteWrappingLabTest`，把“early proxy 成功路径”与“形态不一致 fail-fast（或带隐患继续运行）”跑成断言；再回正文对照 `getSingleton → getEarlyBeanReference → doCreateBean` 的关键分支。
...
**断点入口**：`AbstractAutowireCapableBeanFactory#getEarlyBeanReference`

---

## 先运行实验：让问题变成“可见”的

按以下顺序运行（从“成功路径”到“失败边界”）：
...
这个失败在循环依赖里会更“早暴露”，因为 early 阶段可能就已经获取到了 proxy 形态。
...
3. **“仅引入 AOP 后开始出现循环依赖相关异常”**
  - 代理改变了 final 形态，使 early/raw 与 final/proxy 的不一致暴露出来；这不是 AOP “制造了 bug”，而是它让阅读者观察到了之前已经存在的边界。
