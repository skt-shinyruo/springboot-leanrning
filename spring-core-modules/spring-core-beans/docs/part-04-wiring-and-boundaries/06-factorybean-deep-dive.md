...
上一章：[05. Bean 名称与 alias：同一个实例，多一个名字](05-bean-names-and-aliases.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[07. BeanDefinition 覆盖（overriding）：同名 bean 是“最后一个赢”还是“直接失败”？](07-bean-definition-overriding.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
- `getType("valueFactory")` 更接近在问：**product 的类型是什么？**
- `getType("&valueFactory")` 更接近在问：**factory 的类型是什么？**
...
- “按类型发现/条件装配行为很怪” → **定义层（类型元数据）**：检查 `getObjectType()` 是否可靠（见 [29](12-factorybean-edge-cases.md)）
...
- 能返回明确类型就返回明确类型
- 如果确实无法确定，至少在文档/注释中说明原因，并配套测试覆盖边界（本模块已提供，见 [29. FactoryBean 边界：getObjectType 返回 null 会让“按类型发现”失效](12-factorybean-edge-cases.md) + Lab）
...
- 如果 `getObjectType()` 信息不足（返回 `null`），某些查找路径会选择 **不去实例化 factory**（尤其 `allowEagerInit=false` 时），于是可以观察到“按类型找不到但按名字能获取到”的现象（见 [29. FactoryBean 边界：getObjectType 返回 null 会让“按类型发现”失效](12-factorybean-edge-cases.md)）。
...
上一章：[22. beanName 与 alias：命名规则与别名本质](05-bean-names-and-aliases.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[24. BeanDefinition 覆盖：同名定义的冲突策略](07-bean-definition-overriding.md)
