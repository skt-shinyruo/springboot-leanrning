# 20. 泛型匹配与注入误区：ResolvableType 与代理导致的类型信息丢失
<!-- CHAPTER-CARD:START -->
!!! summary "章节学习卡片（五问闭环）"
    - 使用方式：可先运行本章推荐 Lab，把现象固化为断言，再对照正文理解机制；真实项目里优先按“定义层/实例层/最终暴露对象”分层，再用断点与 watch list 收敛原因。

    本章围绕37. 泛型匹配与注入误区：ResolvableType 与代理导致的类型信息丢失展开，主线可以概括为：`ApplicationContext#refresh` 主线：注册 BeanDefinition → BFPP 加工定义 → 实例化/注入 → BPP 增强（代理/回调）→ 生命周期与销毁。
...
上一章：[19. 类型转换：BeanWrapper / ConversionService / PropertyEditor 的边界](19-type-conversion-and-beanwrapper.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[21. Environment Abstraction：PropertySource / @PropertySource / 优先级与排障主线](21-environment-and-propertysource.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
!!! tip "继续加深：把本章跑成可验证路线"
    ...
    建议 先跑 `SpringCoreBeansGenericTypeMatchingPitfallsLabTest`，再用 `SpringCoreBeansGenericTypeMatchingPitfallsLabTest#genericTypeMatching_canFailWhenTypeInfoIsLost` 做对照；把两次差异对齐到正文的关键分支解释。
    - 第一断点：`GenericTypeAwareAutowireCandidateResolver#checkGenericTypeMatch`（以本章正文“断点建议/证据链”处为准；若本章提供固定观察点，优先按观察点收敛结论）。
    - 本章加深重点：读到“0. 先建立一个“排障口径”：候选类型信息的三大来源”时，建议将“误判点”收敛成更短的分流：现象 → 第一入口 → 关键分支 → 结论，读者可以按步骤自证。
...
2) **FactoryBean 的 product type**：通过 `FactoryBean#getObjectType()`（以及一些预测机制）
   - 优点：能把“复杂构建逻辑”隐藏在工厂里
   - 风险：`getObjectType()` 不可靠/返回 null，会直接让 type discovery 变得不稳定（见 [23](06-factorybean-deep-dive.md)、[29](12-factorybean-edge-cases.md)）
...
3) **FactoryBean 的类型预测**
   - `FactoryBean#getObjectType()` 返回不准确/为 `null` 时，按类型发现会出现边界问题（见 [23. FactoryBean 深潜：product vs factory、类型匹配、以及 isSingleton 缓存语义](06-factorybean-deep-dive.md) 与 [29. FactoryBean 边界：getObjectType 返回 null 会让“按类型发现”失效](12-factorybean-edge-cases.md)）
...
上一章：[36. 类型转换：BeanWrapper / ConversionService / PropertyEditor 的边界](19-type-conversion-and-beanwrapper.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[38. Environment Abstraction：PropertySource / @PropertySource / 优先级与排障主线](21-environment-and-propertysource.md)
