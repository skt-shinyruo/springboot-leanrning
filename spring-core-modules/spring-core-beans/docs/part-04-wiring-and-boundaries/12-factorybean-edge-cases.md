# 12. FactoryBean 边界：getObjectType 返回 null 会让“按类型发现”失效
<!-- CHAPTER-CARD:START -->
!!! summary "章节学习卡片（五问闭环）"
...
上一章：[11. 自定义 Scope + scoped proxy：thread scope 的真实语义](11-custom-scope-and-scoped-proxy.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[13. 注入阶段：field injection vs constructor injection（以及 `postProcessProperties`）](13-injection-phase-field-vs-constructor.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
建议优先运行 `SpringCoreBeansFactoryBeanEdgeCasesLabTest`（或文末“对应 Lab/Test”中的最小入口），再回到正文逐段对照分支与原因。
...
`FactoryBean` 的核心机制读者已经在 [23 章](06-factorybean-deep-dive.md) 学过了。
...
- “在 Boot 条件装配里出现诡异匹配结果” → **定义层 + 条件机制**：FactoryBean 的类型声明不可靠会影响条件判断，建议优先修正 `getObjectType()`（并回看 [10](../part-02-boot-autoconfig/03-spring-boot-auto-configuration.md)）
...
上一章：[28. 自定义 scope 与 scoped proxy：线程 scope 复现](11-custom-scope-and-scoped-proxy.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[30. 注入发生在什么时候：field vs constructor](13-injection-phase-field-vs-constructor.md)
