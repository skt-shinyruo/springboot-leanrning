# 14. 代理产生在哪个阶段：BPP 如何把 Bean 换成 Proxy（以及 self-invocation）
<!-- CHAPTER-CARD:START -->
!!! summary "章节学习卡片（五问闭环）"
    - 使用方式：可先运行本章推荐 Lab，把现象固化为断言，再对照正文理解机制；真实项目里优先按“定义层/实例层/最终暴露对象”分层，再用断点与 watch list 收敛原因。

    本章围绕代理产生在哪个阶段：BPP 如何把 Bean 换成 Proxy（以及 self-invocation）展开，主线可以概括为：`ApplicationContext#refresh` 主线：注册 BeanDefinition → BFPP 加工定义 → 实例化/注入 → BPP 增强（代理/回调）→ 生命周期与销毁。
...
上一章：[13. 注入阶段：field injection vs constructor injection（以及 `postProcessProperties`）](13-injection-phase-field-vs-constructor.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[15. `@Resource` 注入：为什么其定位更接近“按名称找 Bean”？](15-resource-injection-name-first.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
- AOP 前置理解（call path/self-invocation）：[01. AOP：代理（Proxy）+ 入口（Call Path）](../../../spring-core-aop/docs/part-01-proxy-fundamentals/01-aop-proxy-mental-model.md)（为什么要跳：本章解释“容器允许换对象”，AOP 侧补齐“代理到底是什么 + 调用从哪进”；验证什么：在 AOP 章跑一个最小 proxy 用例，确认只有“经过代理的调用”才会触发增强）
- AOP 容器主线（为什么 AutoProxyCreator 是 BPP）：[07. AOP 的容器主线：AutoProxyCreator 作为 BPP](../../../spring-core-aop/docs/part-02-autoproxy-and-pointcuts/01-autoproxy-creator-mainline.md)（为什么要跳：本章看到的是“BPP 把 bean 换成 proxy”，AOP 侧补齐“是谁在 after-init 阶段 wrapIfNecessary”；验证什么：在 `AbstractAutoProxyCreator#postProcessAfterInitialization` 附近观察 proxy 的产生条件与目标对象）
...
1) **pre-instantiation short-circuit（实例化前短路）**
   ...
   - 对应章节：[15](../part-03-container-internals/04-pre-instantiation-short-circuit.md)
2) **early reference（循环依赖窗口）**
   - `SmartInstantiationAwareBeanPostProcessor#getEarlyBeanReference`
   - 对应章节：[16](../part-03-container-internals/05-early-reference-and-circular.md)
...
| 有时是原对象，有时是 proxy | 创建时机不同导致错过/命中 BPP | 对照 `registerBeanPostProcessors` 与目标 bean 创建时机；看是否过早 `getBean` | 避免在 BFPP/BDRPP 阶段过早创建；保证 BPP 链完整 | 结合 [14](../part-03-container-internals/03-post-processor-ordering.md)、[25](08-programmatic-bpp-registration.md) |
...
!!! tip "继续加深：把本章跑成可验证路线"
    ...
    建议 先跑 `SpringCoreBeansBeanCreationTraceLabTest`，再用 `SpringCoreBeansProxyingPhaseLabTest` 做对照；把两次差异对齐到正文的关键分支解释。
...
下一章见：[32. `@Resource` 注入：为什么其定位更接近“按名称找 Bean”？](15-resource-injection-name-first.md)
...
上一章：[30. 注入阶段：field injection vs constructor injection（以及 `postProcessProperties`）](13-injection-phase-field-vs-constructor.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[32. `@Resource` 注入：为什么其定位更接近“按名称找 Bean”？](15-resource-injection-name-first.md)
