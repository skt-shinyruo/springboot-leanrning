# 13. 注入阶段：field injection vs constructor injection（以及 `postProcessProperties`）
<!-- CHAPTER-CARD:START -->
!!! summary "章节学习卡片（五问闭环）"
...
上一章：[12. FactoryBean 边界：getObjectType 返回 null 会让“按类型发现”失效](12-factorybean-edge-cases.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[14. 代理产生在哪个阶段：BPP 如何把 Bean 换成 Proxy（以及 self-invocation）](14-proxying-phase-bpp-wraps-bean.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
!!! tip "继续加深：把本章跑成可验证路线"
    ...
    - 建议入口：先跑 `SpringCoreBeansInjectionPhaseLabTest` 把现象跑出来；跑完后回到正文，把“现象 → 调用链/分支 → 结论”对齐到源码。
...
- [12. 容器启动与基础设施处理器：为什么注解能工作？](../part-03-container-internals/01-container-bootstrap-and-infrastructure.md)
...
- AOP（代理与调用路径）：[spring-core-aop：代理（Proxy）+ 入口（Call Path）](../../../spring-core-aop/docs/part-01-proxy-fundamentals/01-aop-proxy-mental-model.md)（为什么要跳：本章讲“注入发生在哪个阶段”，一旦引入代理，很多现象会变成“对象形态 + 调用路径”的问题；验证什么：在 AOP 章跑一个最小 proxy 用例，确认“字段注入/构造器注入”与“代理是否生效”是两条不同维度）
- 事务也是代理：[spring-core-tx：`@Transactional` 代理](../../../spring-core-tx/docs/part-01-transaction-basics/02-transactional-proxy.md)
...
- “`@Autowired/@Value` 完全不生效” → **优先定义层/基础设施问题**：注解处理器是否注册？（见 [12](../part-03-container-internals/01-container-bootstrap-and-infrastructure.md)）
- “注入发生了但候选选择不符合预期” → **实例层（依赖解析）**：转到 [03](../part-01-ioc-container/02-dependency-injection-resolution.md)/[33](16-autowire-candidate-selection-primary-priority-order.md)
...
上一章：[29. FactoryBean 边界误区：泛型/代理/对象类型推断](12-factorybean-edge-cases.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[31. 代理产生在哪个阶段：BPP 如何把 Bean 换成 Proxy](14-proxying-phase-bpp-wraps-bean.md)
