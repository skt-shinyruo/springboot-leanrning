# 02. dependsOn：强制初始化顺序（即使没有显式依赖）
<!-- CHAPTER-CARD:START -->
!!! summary "章节学习卡片（五问闭环）"
    - 使用方式：可先运行本章推荐 Lab，把现象固化为断言，再对照正文理解机制；真实项目里优先按“定义层/实例层/最终暴露对象”分层，再用断点与 watch list 收敛原因。

    本章围绕dependsOn：强制初始化顺序（即使没有显式依赖）展开，主线可以概括为：`ApplicationContext#refresh` 主线：注册 BeanDefinition → BFPP 加工定义 → 实例化/注入 → BPP 增强（代理/回调）→ 生命周期与销毁。
...
上一章：[01. Lazy：lazy-init bean vs `@Lazy` 注入点（懒代理）](01-lazy-semantics.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[03. registerResolvableDependency：能注入，但它不是 Bean](03-resolvable-dependency.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
- 例如：某个 bean 的 `init` 逻辑依赖“某个外部资源已就绪”，而资源初始化不一定通过 DI 表达（此时更建议显式 DI，但有时历史包袱会逼读者用 dependsOn）。
...
如果 bean A 本身是 lazy-init 或者压根不是单例预实例化覆盖范围（例如 prototype），那么直到读者第一次触发：
...
> 实务建议：若想要“注入不拉起、首次使用才拉起”，用 **注入点 `@Lazy` / `ObjectProvider`**。
...
- 标准答案：因为 dependsOn 的实现方式就是“创建 A 之前显式 `getBean(dep)`”，读者主动触发了依赖的创建。
...
!!! tip "继续加深：把本章跑成可验证路线"
    ...
    建议 先跑 `SpringCoreBeansDependsOnLabTest` 把现象跑出来；跑完后回到正文，把“现象 → 调用链/分支 → 结论”对齐到源码。
...
上一章：[18. @Lazy 的真实语义：延迟的是谁、延迟到哪一步](01-lazy-semantics.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[20. registerResolvableDependency：能注入，但它不是 Bean](03-resolvable-dependency.md)
