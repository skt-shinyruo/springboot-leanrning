...
上一章：[14. 代理产生在哪个阶段：BPP 如何把 Bean 换成 Proxy（以及 self-invocation）](14-proxying-phase-bpp-wraps-bean.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[16. 候选选择 vs 顺序：`@Primary` / `@Priority` / `@Order` / `@Qualifier` 的边界](16-autowire-candidate-selection-primary-priority-order.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
| 报多候选/歧义异常 | name 未命中，fallback 走 type 时遇到多个候选 | 优先指定 name；或回到 [33](16-autowire-candidate-selection-primary-priority-order.md) 的候选规则 |
...
- 参见：[03](../part-01-ioc-container/02-dependency-injection-resolution.md)、[33](16-autowire-candidate-selection-primary-priority-order.md)
...
上一章：[31. 代理产生在哪个阶段：BPP 如何把 Bean 换成 Proxy](14-proxying-phase-bpp-wraps-bean.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[33. 候选选择与优先级：@Primary/@Priority/@Order 的边界](16-autowire-candidate-selection-primary-priority-order.md)
