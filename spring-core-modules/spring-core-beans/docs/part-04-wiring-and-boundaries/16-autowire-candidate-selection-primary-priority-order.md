# 16. 候选选择 vs 顺序：`@Primary` / `@Priority` / `@Order` / `@Qualifier` 的边界
<!-- CHAPTER-CARD:START -->
!!! summary "章节学习卡片（五问闭环）"
...
上一章：[15. `@Resource` 注入：为什么其定位更接近“按名称找 Bean”？](15-resource-injection-name-first.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[17. `@Value("${...}")` 占位符解析：默认 non-strict vs strict fail-fast](17-value-placeholder-resolution-strict-vs-non-strict.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
- 本章主题：**16. 候选选择 vs 顺序：`@Primary` / `@Priority` / `@Order` / `@Qualifier` 的边界**
...
- 下一章：[34. `@Value("${...}")` 占位符解析：默认 non-strict vs strict fail-fast](17-value-placeholder-resolution-strict-vs-non-strict.md)
...
上一章：[32. `@Resource` 注入：为什么其定位更接近“按名称找 Bean”？](15-resource-injection-name-first.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[34. `@Value("${...}")` 占位符解析：默认 non-strict vs strict fail-fast](17-value-placeholder-resolution-strict-vs-non-strict.md)
