...
## 定位：Appendix 工具页的深化方式

Appendix 不是新的主线教程，而是训练、定位和复盘工具。深化时要减少抽象说明，把每个工具页改成能直接使用的入口：现象如何定位、答案如何证明、题目如何训练复述。
...
Appendix 的再加深重点，是把工具页做成可复用的训练与排障中枢：能从症状快速定位，能把答案用证据链证明，也能用题库训练复述。

## 执行化提示（工具页的“复用价值”）

- 工具页不追求讲新概念，而是降低定位成本。每一项都应链接到章节正文、断点入口或 Lab，形成可导航闭环。
- 工具页的最低标准，是给出“它在证明什么”“如何观察到证据”“最常见误诊反例是什么”；否则工具页很难在真实排障中复用。
