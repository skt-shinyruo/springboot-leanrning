...
上一章：[01. 异步监听器：`@Async` 生效需要什么？线程会怎么变？](01-async-listener.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[03. `@TransactionalEventListener`：为什么 after-commit 事件能“等事务提交后再执行”？](03-transactional-event-listener.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
以为“事件已经异步”，但实际只有某个 listener 异步；或者以为发布方不会被影响，结果仍被同步 listener 拖慢/异常打断
...
先选清楚需要异步的是“某个 listener”还是“整个分发过程”，再用可断言的线程名/执行时机把行为锁定
...
上一章：[05-async-listener](01-async-listener.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[07-transactional-event-listener](03-transactional-event-listener.md)
