...
上一章：[04. 同步与异常传播：为什么监听器抛异常会“炸到发布方”？](../part-01-event-basics/04-sync-and-exceptions.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[02. 异步广播：让事件“默认异步”而不是靠 `@Async`](02-async-multicaster.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
给 listener 方法加了 `@Async`，但断点/日志显示仍然在发布事件的线程里执行。

`@Async` 依赖 Spring 的代理机制；如果没有开启 `@EnableAsync`（或 bean 没被代理、发生自调用），`@Async` 会被忽略。

`SpringCoreEventsMechanicsLabTest#asyncAnnotationIsIgnored_withoutEnableAsync`、`SpringCoreEventsMechanicsLabTest#asyncListenerRunsOnDifferentThread_whenEnableAsyncIsOn`
...
上一章：[04-sync-and-exceptions](../part-01-event-basics/04-sync-and-exceptions.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[06-async-multicaster](02-async-multicaster.md)
