...
上一章：[02. 多监听器与顺序：为什么 `@Order` 值得认真对待？](event-basics-multiple-listeners-and-order.md) ｜ 目录：[模块目录](../README.md) ｜ 下一章：[04. 同步与异常传播：为什么监听器抛异常会“炸到发布方”？](event-basics-sync-and-exceptions.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
先用最小 payload（如 String）验证类型匹配，再逐步升级为专用 event class（可采用 immutable record）
...
上一章：[02-multiple-listeners-and-order](event-basics-multiple-listeners-and-order.md) ｜ 目录：[模块目录](../README.md) ｜ 下一章：[04-sync-and-exceptions](event-basics-sync-and-exceptions.md)
