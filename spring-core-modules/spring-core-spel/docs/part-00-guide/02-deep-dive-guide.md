...
- 本章主题：**02. 深挖导读：把“表达式求值”落到源码与断点**
- 建议入口：优先运行 `SpringCoreSpelLabTest`，以获得可回归的现象与断言入口。
- 阅读目标：SpEL 的可解释性来自 AST 与 context：同一表达式在不同 context 下可能得到不同结果。
- 源码入口：`SpelExpressionParser#parseExpression` /（AST）`SpelNodeImpl` / `Expression#getValue`

## 小结与下一章

- 小结：SpEL 的可解释性来自 AST 与 context：同一表达式在不同 context 下可能得到不同结果。
- 下一章：[第 210 章：01：SpEL 调用链](03-spel-call-chain.md)
