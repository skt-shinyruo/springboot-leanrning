...
上一章：[04. 断点地图（SpEL）](guide-breakpoint-map.md) ｜ 目录：[模块目录](../README.md) ｜ 下一章：[01. SpEL 入门（root/variables/property access）](spel-basics.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
本章完成后，应能复述：root/variables/context 决定求值结果；同一表达式在不同 context 下可能不同。需要下探源码时，可以从 `SpelExpression#getValue` / `StandardEvaluationContext` 这些入口切入。
...
下一章见：[01：SpEL 入门](spel-basics.md)
...
上一章：[guide-breakpoint-map.md](guide-breakpoint-map.md) ｜ 目录：[模块目录](../README.md) ｜ 下一章：[spel-basics.md](spel-basics.md)
