...
上一章：[05. 读取资源：InputStream、编码与“可观察性”](resource-abstraction-reading-and-encoding.md) ｜ 目录：[模块目录](../README.md) ｜ 下一章：[01. 常见坑清单（排查时对照）](appendix-common-pitfalls.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
这一章解决的本质上是一个**误会**：把 `Resource` 当成了 `File`。

在 IDE 里，这个误会经常“侥幸成立”——因为 `src/main/resources` 会被复制到 `target/classes`，表面上就像普通文件夹；
...
- 需要调试时看 `resource.getDescription()`（它会把“当前到底拿到了什么”说得更清楚）

### 3) pattern 扫描时：`classpath:` 与 `classpath*:` 的差异

如果在做“扫描多个资源文件”，形成肌肉记忆：
...
- 把观察记录成两条结论：`getInputStream()` 稳定；`getFile()` 取决于资源是否真的是文件
...
上一章：[05-reading-and-encoding](resource-abstraction-reading-and-encoding.md) ｜ 目录：[模块目录](../README.md) ｜ 下一章：[90-common-pitfalls](appendix-common-pitfalls.md)
