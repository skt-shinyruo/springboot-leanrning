...
上一章：[03. 方法参数校验：为什么它必须依赖 Spring 代理？](validation-core-method-validation-proxy.md) ｜ 目录：[模块目录](../README.md) ｜ 下一章：[05. 自定义约束：如何写一个最小可用的 `@Constraint`？](validation-core-custom-constraint.md)
<!-- GLOBAL-BOOK-NAV:END -->

## 导读

本章围绕「04. Groups：同一个对象，为什么“创建”和“更新”要用不同规则？」展开，目标是把机制边界写成可回归的事实（可运行入口与关键观察点会在文中给出）。
优先运行 `SpringCoreValidationMechanicsLabTest`（或文末“对应实验/测试”中的最小入口），再回到正文逐段对照分支与原因。
...
上一章：[03-method-validation-proxy](validation-core-method-validation-proxy.md) ｜ 目录：[模块目录](../README.md) ｜ 下一章：[05-custom-constraint](validation-core-custom-constraint.md)
