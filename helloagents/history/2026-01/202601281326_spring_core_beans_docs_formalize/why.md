# Change Proposal: spring-core-beans docs 书面化改写（去口语化）

## Requirement Background

`spring-core-modules/spring-core-beans/docs` 的技术深度已覆盖主要机制域，但正文中仍存在较多“口语化/对话式”的表达（例如大量第二人称叙述、俚语化表述与课堂化措辞）。这类表达在“源码进阶/面试复述/团队内训讲义化”场景下，会带来两个问题：

1. **可复述性下降：** 读者难以将内容直接迁移为书面答案（面试/文档/评审口径），需要额外“转述”成本。
2. **严谨性观感受损：** 同样的机制描述，在口语语境下容易被误读为“经验性建议”，不利于建立“基于源码分支/证据链可验证”的论证风格。

因此需要对 `spring-core-beans` 模块 docs 进行一次“书面化处理”，在不降低信息密度与机制深度的前提下，统一去除口语化表达，形成更适合教学交付与复盘的文本风格。

## Change Content

1. 对 `spring-core-modules/spring-core-beans/docs/**/*.md` 全量执行“书面化改写”：减少第二人称叙述、去除俚语与口语化提示语，将表达改为“陈述式/规范式/可复述式”。
...
- **Files:** `spring-core-modules/spring-core-beans/docs/**/*.md`
- **APIs:** 无
- **Data:** 无
