...
首次使用时，先在本地完整运行一次测试，以确认 JDK/Maven 环境与仓库依赖可用。这个命令只回答一个问题：当前工作区能否把默认实验全部跑绿。
...
- 只构建/测试某个模块（示例：`spring-boot-web-mvc`）。这个命令用于把排查范围压到一个模块，避免全仓测试输出淹没具体失败：

```bash
mvn -q -pl :spring-boot-web-mvc test
```

- 运行某个模块（示例：`spring-boot-basics`）。这个命令用于观察真实应用启动、配置加载与端口行为：
