...
- 一句话总结：Common Pitfalls（springboot-observability） —— 建议先跑本章推荐 Lab，把现象固化为断言，再对照正文理解机制；真实项目里常用方式：建议先跑本章推荐 Lab，把现象固化为断言，再对照正文主线理解用法。
...
这一章围绕「Common Pitfalls（springboot-observability）」展开：先把边界说清楚，再沿主线推进到关键分支，最后用可运行入口把结论验证出来。
...
- 观察点 1：运行本章推荐入口后，聚焦「Common Pitfalls（springboot-observability）」的生效时机/顺序/边界；断点/入口：（以本章正文“源码；断言：你能解释“为什么此处生效/为什么此处不生效”。
- 观察点 2：运行本章推荐入口后，聚焦「Common Pitfalls（springboot-observability）」的生效时机/顺序/边界；断点/入口：断点”小节为准）；断言：你能解释“为什么此处生效/为什么此处不生效”。
...
上一章：[HTTP metrics：`http.server.requests` 从哪里来](../part-01-observability-basics/206-01-http-metrics-and-observations.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[自检](208-99-self-check.md)
