...
这一章围绕「Self Check（springboot-cache）」展开：先把边界说清楚，再沿主线推进到关键分支，最后用可运行入口把结论验证出来。
...
- Lab：`BootCacheLabTest` / `BootCacheBookMatrixLabTest` / `BootCacheBranchMatrixLabTest` / `BootCacheSpelKeyLabTest`
- Exercise：`BootCacheExerciseTest`

上一章：[常见坑](114-90-common-pitfalls.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[Docs TOC](../README.md)
