...
- 可跑入口（Solutions - 本模块答案回归）：`mvn -q -pl :springboot-cache -Dtest=*ExerciseSolutionTest test`
- 并发/性能专题（可复现实验范式）：[performance-and-concurrency.md](../../book/performance-and-concurrency.md)
- 可跑入口（并发/性能 Lab - 缓存击穿（stampede）可断言复现）：`mvn -q -pl :springboot-cache -Dtest=BootCacheStampedeProtectionLabTest test`
