...
本目录是一条可顺读的教材主线。它不替代各模块正文，也不尝试把 Spring 的机制讲完；它只把分散在模块里的可运行证据链按章节串成路线。

每章都从一个可以运行的入口开始，把关键现象先固定成事实，再把读者送到对应模块的 README、机制正文、分支矩阵或断点地图。遇到具体症状时，不必从第一章重读，可以直接进入 [`90 排障索引`](90-troubleshooting-index.md)。
