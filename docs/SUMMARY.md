...
- 从“封面”（根 `README.md`）开始，先完成一个模块的最小闭环，再按目录继续补齐机制与边界。
- 每个模块的根 `README.md` 是该模块的目录页与阅读顺序（唯一顺序来源）；模块内 `docs/` 目录保持扁平。

**对维护者：**

- `<!--nav-->` **之后**的列表会被文档站解析为侧边栏导航；如需调整目录结构，请在该区域修改。
- 链接使用相对 `docs_dir` 的路径；本仓库的 `docs_dir` 是仓库根目录。
- 文档正文以各模块 `*/docs/*.md` 为准；仓库根 `docs/` 包含 `SUMMARY.md` + `book/`（全书主线），其中 `SUMMARY.md` 作为全站导航（SSOT）。

<!--nav-->
- [封面](README.md)
- [写作规范（维护者）：教材式 / 中性口吻](docs/writing-style-guide.md)
- 全书主线（教材式顺读）
    - [全书主线（总览）](docs/book/README.md)
    - [01. 入门：如何在本仓库学习（入口与证据链）](docs/book/01-getting-started.md)
    - [02. Spring Boot Basics：启动、配置与最小闭环](docs/book/02-spring-boot-basics.md)
    - [03. Spring Core Beans：IoC 容器、Bean 生命周期与扩展点](docs/book/03-spring-core-beans.md)
    - [04. Spring Core AOP：切面、代理与“边界”](docs/book/04-spring-core-aop.md)
    - [05. Spring Core Tx：事务边界、传播与回滚](docs/book/05-spring-core-tx.md)
    - [06. Spring Boot Web MVC：请求主链路与错误形状](docs/book/06-spring-boot-web-mvc.md)
    - [07. Spring Core Validation：约束、Violation 与方法校验边界](docs/book/07-spring-core-validation.md)
    - [08. Spring Boot Testing：切片、全量上下文与证据链](docs/book/08-spring-boot-testing.md)
    - [09. Spring Boot Data JPA：Persistence Context、flush 与脏检查](docs/book/09-spring-boot-data-jpa.md)
    - [10. Spring Boot Web Client：超时/重试与可测试性](docs/book/10-spring-boot-web-client.md)
    - [11. Spring Boot Async & Scheduling：线程边界与上下文传播](docs/book/11-spring-boot-async-scheduling.md)
    - [12. Spring Boot Cache：缓存语义、key 与边界](docs/book/12-spring-boot-cache.md)
    - [13. Observability & Actuator：日志/指标/端点与排障入口](docs/book/13-observability-and-actuator.md)
    - [14. Spring Boot Security：401/403、链路与常见陷阱](docs/book/14-spring-boot-security.md)
    - 附录
        - [90. 排障索引：从现象到最短证据链入口](docs/book/90-troubleshooting-index.md)
        - [91. 术语表：术语对照表](docs/book/91-glossary.md)
        - [92. 参考资料：参考资料与延伸阅读](docs/book/92-references.md)
- 模块文档
    - Spring Boot（应用层）
