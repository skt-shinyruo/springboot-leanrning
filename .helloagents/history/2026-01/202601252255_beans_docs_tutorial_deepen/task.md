...
- [√] 重写 `docs/part-01-ioc-container/08-circular-dependencies.md`（现象分类 → 窗口期 → 三层缓存语义 → 断点闭环 → 工程策略）
- [√] 重写 `docs/part-03-container-internals/05-early-reference-and-circular.md`（early reference 形态一致性：getEarlyBeanReference / raw vs wrapped）
- [√] 重写 `docs/appendix/08-explore-debug-tests.md`（Explore gate 启用方式 + 观察点/断点 + 用回主线）
