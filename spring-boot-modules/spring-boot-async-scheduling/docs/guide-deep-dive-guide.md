# 02. 00 - Deep Dive Guide（springboot-async-scheduling）
<!-- CHAPTER-CARD:START -->
!!! summary "章节学习卡片（深挖导读）"
...
上一章：[01. 主线时间线：Spring Boot Async & Scheduling](guide-mainline-timeline.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[01. `@Async` 心智模型：代理与线程切换](async-scheduling-async-proxy-mental-model.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
所以这模块的取向是：尽量少用“靠感觉的解释”，尽量多给可验证的入口（对应的 `*LabTest#method`）。不是为了让人写更多测试，而是为了让人在排障时不至于只能靠日志猜。
...
如果代理不存在、调用绕开代理、或者 executor 选的不是直觉里的那个，看到的现象就会开始变形。
...
- 想顺着读一篇文章：从 [第 119 章](async-scheduling-async-proxy-mental-model.md) 开始，按主线向下走。
- 现在就想快速确认“这些结论是不是真的”：跑 `BootAsyncSchedulingBookMatrixLabTest`。
...
如果愿意跟一遍源码，不必急于把整个包都翻完。异步与调度这条链路，最常用的入口其实就这三个：
...
下一章见：[第 119 章：01：`@Async` 心智模型：代理与线程切换](async-scheduling-async-proxy-mental-model.md)
...
上一章：[Docs TOC](../README.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[part-01-async-scheduling/01-async-proxy-mental-model.md](async-scheduling-async-proxy-mental-model.md)
