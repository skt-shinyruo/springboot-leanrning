...
1. 先把 `@Async` 的“生效点”讲明白：代理与线程切换
   - 阅读：[01：`@Async` 心智模型：代理与线程切换](async-scheduling-async-proxy-mental-model.md)
...
4. 把最常见的坑单独拎出来：self-invocation
   - 阅读：[04：self-invocation：为什么异步有时不生效](async-scheduling-self-invocation.md)
   - 为什么要单独一章：这不是 `@Async` 的“特殊规则”，而是 AOP 的通用边界；懂了它，`@Transactional`、校验、权限这些也顺带更清晰。

5. 接着进入 `@Scheduled`：它不是“调用期拦截”，而是“启动期注册 + 运行期触发”
   - 阅读：[05：`@Scheduled` 基础与可测试性](async-scheduling-scheduling-basics.md)
   - 为什么放在后面：调度的坑更接近“时间相关 + 线程相关”的组合题，先把 `@Async` 的线程模型打稳更稳妥。

6. 最后补上三章工程边界：事务、上下文、Boot 自动装配
   - [06：`@Async` × `@Transactional`：事务边界与执行线程](async-scheduling-async-and-transactions.md)
   - [07：SecurityContext / RequestContext：默认丢失、传播与泄漏](async-scheduling-security-and-request-context.md)
   - [08：Spring Boot `spring.task.*`：默认线程池/调度器与属性映射](async-scheduling-boot-spring-task-autoconfig.md)
