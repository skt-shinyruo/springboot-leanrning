...
上一章：[01. 主线时间线：Spring Boot Async & Scheduling](01-mainline-timeline.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[01. `@Async` 心智模型：代理与线程切换](../part-01-async-scheduling/01-async-proxy-mental-model.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
- 想顺着读一篇文章：从 [第 119 章](../part-01-async-scheduling/01-async-proxy-mental-model.md) 开始，按主线向下走。
- 现在就想快速确认“这些结论是不是真的”：跑 `BootAsyncSchedulingBookMatrixLabTest`。
- 正在排障（不生效/线程不对/异常不见了）：跑 `BootAsyncSchedulingBranchMatrixLabTest`，然后去看 [断点地图](04-breakpoint-map.md) 与 [关键分支矩阵](05-branch-decision-matrix.md)。
...
更细的断点清单在 [断点地图](04-breakpoint-map.md)，其定位更接近“排障用的备忘录”。
