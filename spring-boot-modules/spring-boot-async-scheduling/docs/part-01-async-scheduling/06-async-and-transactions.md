...
因此，一旦把执行切到另一个线程，原线程的事务上下文就不会“自动跟过去”。
...
当一个方法同时标注 `@Async` 与 `@Transactional`，真正关心的不是“有没有事务”，而是：

> 事务是在调用方线程开启，还是在异步线程开启？

这个分支只需要一个可验证的判断：

- 如果事务在异步线程：在异步线程里观察到 `TransactionSynchronizationManager.isActualTransactionActive()==true`
...
- 以为“在调用方事务里丢一个 @Async 做异步写库，失败就回滚”
