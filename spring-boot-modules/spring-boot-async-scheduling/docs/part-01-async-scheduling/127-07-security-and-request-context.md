# 第 127 章：07：SecurityContext / RequestContext：默认丢失、传播与泄漏
<!-- CHAPTER-CARD:START -->
!!! summary "章节学习卡片（上下文丢失的真相）"

    如果你在异步线程里拿不到“当前用户”或“当前请求”，先别急着怀疑 Spring Security 或 MVC：它们大概率只是 ThreadLocal 的受害者。
...
上一章：[第 126 章：06：`@Async` × `@Transactional`：事务边界与执行线程](126-06-async-and-transactions.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[第 128 章：08：Spring Boot `spring.task.*`：默认线程池/调度器与属性映射](128-08-boot-spring-task-autoconfig.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
上一章：[part-01-async-scheduling/06-async-and-transactions.md](126-06-async-and-transactions.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[part-01-async-scheduling/08-boot-spring-task-autoconfig.md](128-08-boot-spring-task-autoconfig.md)
