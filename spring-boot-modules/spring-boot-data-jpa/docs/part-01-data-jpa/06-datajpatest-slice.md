...
以为数据会落库/对外可见，但测试结束后一切“消失”；或在测试里做跨事务验证一直不稳定

`@DataJpaTest` 默认用事务包裹并在测试结束回滚，这对学机制很友好，但不等价于生产运行时

`BootDataJpaLabTest#dataJpaTestRunsInsideATransaction`

学机制优先 `@DataJpaTest`；需要跨层/跨事务的真实边界验证，再切换到 `@SpringBootTest` 或在专门用例里明确事务策略
