...
上一章：[04. Dirty Checking（脏检查）：为什么改字段不需要 save 也能落库？](data-jpa-dirty-checking.md) ｜ 目录：[模块目录](../README.md) ｜ 下一章：[06. `@DataJpaTest`：为什么它适合学 JPA（切片测试）](data-jpa-datajpatest-slice.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
这类问题最怕“只看代码不看 SQL”。在这个模块里，至少形成两条证据链：

- 用 `BootDataJpaDebugSqlLabTest` 把 SQL 打开（让“预期只有一条查询”变成可见事实）
...
这一章以“学习路线图”的方式讲 N+1（本模块目前通过 练习 引导亲手复现）。
...
学习时先把问题复现清楚，再讨论常见解决手段（fetch join / entity graph / batch size 等）。
...
上一章：[data-jpa-dirty-checking.md](data-jpa-dirty-checking.md) ｜ 目录：[模块目录](../README.md) ｜ 下一章：[data-jpa-datajpatest-slice.md](data-jpa-datajpatest-slice.md)
