...
上一章：[第 111 章：03：key / condition / unless：缓存边界](111-03-key-condition-unless.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[第 113 章：05：过期与可测性：用 Ticker 控制时间](113-05-expiry-with-ticker.md)
<!-- GLOBAL-BOOK-NAV:END -->

## 这类问题你一定见过：并发一上来，下游就被打爆

缓存的命中路径很快，但未命中路径往往很重（回源 DB/远程调用）。当某个热点 key 在同一时间被并发请求，如果每个线程都去做一次“未命中计算”，你会得到：
...
`sync=true` 不是“让缓存更快”，它是“用等待换重复计算”。如果你的计算很轻、或者 key 分布非常散，开启它可能只会增加等待与资源占用。
