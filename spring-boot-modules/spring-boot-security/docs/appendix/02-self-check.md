...
上一章：[01. 常见坑清单（Security）](01-common-pitfalls.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[Docs TOC](../README.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
1. 401 与 403 的分界线是什么？你如何用两条用例把它固定为事实（而不是靠口述）？  
...
2. Basic Auth 成功时，认证信息在哪里体现？你如何验证“当前用户是谁”？  
   - 证据入口：`BootSecurityLabTest#secureEndpointIsAccessibleWithBasicAuth`
3. admin 端点对 admin 用户放行的条件是什么？你如何证明“不是所有带用户名的人都能进”？  
...
8. scope/权限不足时为什么会 403？你如何用一对用例把“read vs admin”的差异跑成事实？  
...
9. 多条 `SecurityFilterChain` 时，请求是如何被 matcher 分流的？你如何证明 `/api/jwt/**` 不会走 Basic chain？  
   - 证据入口：`BootSecurityMultiFilterChainOrderLabTest#jwtPathMatchesJwtChain_andApiPathMatchesBasicChain`
10. method security 的边界在哪里？为什么 self-invocation 会绕过它？你如何用一个可回归用例锁定这个坑？  
    - 证据入口：`BootSecurityLabTest#selfInvocationBypassesMethodSecurityAsAPitfall`

## 退出条件（完成标准）

- 你能把“安全问题”拆成可验证分支：路由/暴露（404）→ 认证（401）→ 授权（403）→ CSRF（403 的一种）。
- 你能把多 filter chain 与 method security 的两条链路区分开，并能指回对应的测试证据。
