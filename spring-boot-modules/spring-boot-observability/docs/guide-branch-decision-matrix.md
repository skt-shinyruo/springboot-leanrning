...
上一章：[04. 断点地图（Observability）](guide-breakpoint-map.md) ｜ 目录：[模块目录](../README.md) ｜ 下一章：[01. HTTP metrics（`http.server.requests` 从哪里来）](observability-basics-http-metrics-and-observations.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
本章完成后，应能复述：请求链路存在 → observation/meter 记录 → 指标可查询。需要下探源码时，可以从 `DispatcherServlet#doDispatch` / `MeterRegistry` 这些入口切入。
...
下一章见：[01：HTTP metrics](observability-basics-http-metrics-and-observations.md)
...
上一章：[guide-breakpoint-map.md](guide-breakpoint-map.md) ｜ 目录：[模块目录](../README.md) ｜ 下一章：[observability-basics-http-metrics-and-observations.md](observability-basics-http-metrics-and-observations.md)
