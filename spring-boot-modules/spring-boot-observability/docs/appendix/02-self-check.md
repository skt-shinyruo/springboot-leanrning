...
- 你能把“观察（metrics）”落到可回归事实：跑请求 → 指标出现/计数变化 → 写断言。
- 你能解释 tag 的收益与风险，并能给出一个“可控 tag 策略”（有断言/过滤器兜底）。
