...
上一章：[03. 请求调用链速览（从 FilterChain 到 DispatcherServlet#doDispatch）](03-webmvc-request-call-chain.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[01. HandlerMapping：路由、404/405 与 mapping 约束](../03-handlermapping/01-handlermapping-routing.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
2. **handler 抛异常（500）**：如果的 `@ControllerAdvice`/resolver 兜不住，最终也会回落到 `/error`
   - 章节承接：错误页与 Accept 分支详见 Part 02：[03：错误页（error/*.html）与内容协商（Accept：HTML vs JSON）](../11-boot-error/03-error-pages-and-content-negotiation.md)
...
- 章节：[05：Interceptor 的 async 生命周期（为什么会回调两次）](../12-async-sse/05-interceptor-async-lifecycle.md)
