...
优先运行 `BootWebMvcAsyncSseLabTest`（或文末“对应实验/测试”中的最小入口），再回到正文逐段对照分支与原因。
...
上一章：[01. Servlet Async（Callable）与测试（asyncDispatch）](async-sse-servlet-async-and-testing.md) ｜ 目录：[模块目录](../README.md) ｜ 下一章：[03. DeferredResult（回调式异步）与 timeout/fallback（可控分支）](async-sse-deferredresult-and-timeout.md)
<!-- BOOKIFY:END -->
