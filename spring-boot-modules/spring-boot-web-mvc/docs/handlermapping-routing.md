...
## 补充：路径匹配策略（PathPatternParser vs AntPathMatcher）
...
- `RequestMappingInfo#getActivePatternsCondition()` 的实际类型不同
  - PathPatternParser：`PathPatternsRequestCondition`
  - AntPathMatcher：`PatternsRequestCondition`
- `RequestMappingHandlerMapping` 上的 parser 是否为 `null`
  - `RequestMappingHandlerMapping#getPatternParser() != null` 通常意味着 PathPatternParser 生效
...
- 断点地图：[`14-testing-observability/06-breakpoint-map.md`](testing-observability-breakpoint-map.md)
- 测试与排障：[`14-testing-observability/01-webmvc-testing-and-troubleshooting.md`](testing-observability-webmvc-testing-and-troubleshooting.md)
