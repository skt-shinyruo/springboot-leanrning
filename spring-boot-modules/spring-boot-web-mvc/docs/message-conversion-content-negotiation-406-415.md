...
上一章：[01. 校验（Validation）与错误响应形状（Error Shape）](binding-validation-validation-and-error-shaping.md) ｜ 目录：[模块目录](../README.md) ｜ 下一章：[02. Jackson ObjectMapper 可控（严格模式、未知字段、时间）](message-conversion-jackson-objectmapper-controls.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
- 本章先运行 `BootWebMvcErrorBranchMatrixLabTest` 固定三类分支（同一视角对照 400/406/415）：
