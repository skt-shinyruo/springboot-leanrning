...
上一章：[03. HttpMessageConverter 与返回值处理（序列化发生在哪里）](return-value-view-message-converters-and-return-values.md) ｜ 目录：[模块目录](../README.md) ｜ 下一章：[03. 错误契约加固（解析失败 vs 校验失败 vs 类型不匹配）](exception-resolvers-error-contract-hardening.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
本章把“坏输入”分成两类，并解释它们为什么表面上都可能是 400，但根因不同：
...
更完整的 resolver 链解释见：
- [`04-exception-resolvers-and-error-flow.md`](exception-resolvers-and-error-flow.md)
...
把“异常处理”拆成两步定位（先定根因，再看谁处理）：

### 1) 先定根因：异常来自哪一段（binder / validation / converter）

- 入口（resolvedException 固化根因）：
...
处理方式：先用测试把三类分开固化（status + message + fieldErrors），再谈“统一错误体”。
...
动作：在 LabTest 里显式 `@Import` advice，避免“偶然生效”。

### 3) ControllerAdvice 的匹配规则（为什么 advice 不生效）
...
- 处理的异常类型不对（异常发生在 converter/binder 阶段，而不是预期中的业务异常）

动作：第一步永远是拿证据——用 `MvcResult#getResolvedException()` 固定异常类型，再回头调整 `@ExceptionHandler`。
