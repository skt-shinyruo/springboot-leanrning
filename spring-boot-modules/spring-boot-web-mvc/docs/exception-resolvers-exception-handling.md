...
上一章：[03. HttpMessageConverter 与返回值处理（序列化发生在哪里）](return-value-view-message-converters-and-return-values.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[03. 错误契约加固（解析失败 vs 校验失败 vs 类型不匹配）](exception-resolvers-error-contract-hardening.md)
<!-- GLOBAL-BOOK-NAV:END -->
