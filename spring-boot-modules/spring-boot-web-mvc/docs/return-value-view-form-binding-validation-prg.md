...
表单闭环的“机制段”主要在 binder 与 view 路径上（Spring Framework `6.2.15`）：

1. `@ModelAttribute` 绑定：`org.springframework.web.servlet.mvc.method.annotation.ServletModelAttributeMethodProcessor#resolveArgument` → `org.springframework.web.bind.WebDataBinder#bind`
2. `@Valid` 校验：`org.springframework.validation.DataBinder#validate`
3. 回显分支（不抛异常）：`org.springframework.validation.BindingResult`（参数顺序决定“走异常流程”还是“回到 view 渲染”）
4. redirect/flash：`org.springframework.web.servlet.view.RedirectView#renderMergedOutputModel`（PRG 的写回阶段）
