...
上一章：[02. ArgumentResolver 与 Binder（参数从哪来、校验在哪触发）](../05-argument-resolver/02-argument-resolver-and-binder.md) ｜ 目录：[Docs TOC](../README.md) ｜ 下一章：[01. 校验（Validation）与错误响应形状（Error Shape）](01-validation-and-error-shaping.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
本模块的目标是：把这些分支都“塑形”为可控的响应（例如 `ApiError`），并用测试锁定它。
...
配合断点地图一起用更省时间：[06-breakpoint-map.md](../14-testing-observability/06-breakpoint-map.md)
