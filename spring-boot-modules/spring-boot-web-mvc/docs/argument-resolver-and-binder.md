...
!!! summary "章节入口（五问闭环）"
    ...
    读完后应能回答：400 到底是缺参/类型不匹配、绑定失败、JSON 解析失败，还是校验失败？以及每一种分支分别在哪个断点上出现。
<!-- CHAPTER-CARD:END -->

<!-- GLOBAL-BOOK-NAV:START -->
上一章：[04. Interceptor 与 Filter：入口在哪里、顺序怎么理解](handleradapter-interceptor-interceptor-and-filter-ordering.md) ｜ 目录：[模块目录](../README.md) ｜ 下一章：[03. 请求绑定（Binding）与 Converter/Formatter](binding-validation-binding-and-converters.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
- `org.springframework.web.method.support.HandlerMethodArgumentResolverComposite#resolveArgument`：总入口（调试时最关键的断点）
...
2. **自定义 Converter/Formatter（把 String 变成当前类型）**
...
这类 400 的共同特点是：**根本还没有进入业务方法体**，且通常可以在 `resolveArgument`/binder 断点上直接看到失败原因。
