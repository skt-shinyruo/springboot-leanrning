...
!!! summary "章节入口（五问闭环）"
    ...
    这页收集 Web Client 里最容易“只在真实环境才暴露”的坑：状态码分流、timeout/cancel、retry 的副作用，以及 filter 链路的顺序预期错误。它们往往不影响本地 200 的 demo，却会在第一次线上波动时导致“到底该不该重试/为什么日志顺序反了”的困惑。

    先运行 `BootWebClientRestClientLabTest` 与 `BootWebClientWebClientLabTest`。如果想更快把分支跑全，可以先跑 Book/Branch Matrix，把 4xx/5xx/timeout/retry 的行为固化成断言，然后再回到本章对照原因与修法。
<!-- CHAPTER-CARD:END -->

<!-- GLOBAL-BOOK-NAV:START -->
上一章：[05. 测试策略：为什么用 MockWebServer？](web-client-testing-with-mockwebserver.md) ｜ 目录：[模块目录](../README.md) ｜ 下一章：[自检题](appendix-self-check.md)
<!-- GLOBAL-BOOK-NAV:END -->
...
为了避免只凭日志猜测测，先把两组矩阵测试跑一遍，让分支变成可重复的断言：
...
需要下探源码时，再对照本模块的断点地图与关键分支矩阵去命中入口（filter 链与 exchange 是高频落点）：[guide-breakpoint-map.md](guide-breakpoint-map.md) / [guide-branch-decision-matrix.md](guide-branch-decision-matrix.md)。
...
> 这些坑很少在“能跑通”时暴露，往往在状态码分支、超时、重试与副作用上翻车。用 MockWebServer 把异常分支跑成断言。
...
`WebClient` 的 filter 本质上是对 `ExchangeFunction` 的一层层包裹：request 走外→内，response 信号回流时是内→外（因此表面上像 response 侧顺序反转）。
...
写 filter 时区分 request/response 侧的执行顺序，并把“期望顺序”直接写进 实验/测试，避免靠脑补与靠经验。
...
上一章：[web-client-testing-with-mockwebserver.md](web-client-testing-with-mockwebserver.md) ｜ 目录：[模块目录](../README.md) ｜ 下一章：[appendix-self-check.md](appendix-self-check.md)
